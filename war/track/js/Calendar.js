...
Calendar.prototype.setDateAsString = function(ymd)
{
    if (ymd) {
        ...
        var y    = (s1 > 0)? parseInt(ymd.substring(0   ,s1)) : 0;
        var m    = (s2 > 0)? parseInt(ymd.substring(s1+1,s2)) : 0;
        var d    = (s2 > 0)? parseInt(ymd.substring(s2+1   )) : 0;
        var YYYY = (y <= 0)? calCurrentYear   : (y < 1900)? 1900 : y;
        var MM1  = (m <= 0)? calCurrentMonth1 : (m >   12)?   12 : m;
        var maxD = this.daysInMonth(YYYY, MM1); // [2.6.4-B12]
        var DD   = (d <= 0)? calCurrentDay    : (d > maxD)? maxD : d;
        this.setDate(YYYY, MM1, DD);
    }
};
