  ...
  //private static       boolean UPDATE_DEVICE              = false;
    ...
    **/
    ...
    private byte[] parseInsertRecord_Device_1(String s)
    {
        ...
        //   |    |     |          | |         |>N|S
        //   |    |     |          | |>Latitude DDmm.mmmm
        //   |    |     |          |>A=ValidGPS, V=InvalidGPS
        //   |    |     |>HHMMSS
        //   |    |>MobileID
        //   |>$POS: POS=PositionData
        Print.logInfo("Parsing: " + s);
        ...
        /* status code */
        int      statusCode = StatusCodes.STATUS_LOCATION;
        if (StringTools.isBlank(eventCode)) {
            // -- blank, leave as-is
        } else
        ...
        if (eventCode.startsWith("0x") || eventCode.startsWith("0X")) {
            // -- specific status code (specified as a hex value)
            int sc = StringTools.parseInt(eventCode,-1);
            if ((sc > 0) && (sc <= 0xFFFF)) {
                statusCode = sc;
            }
        } else
        if (Character.isDigit(eventCode.charAt(0))) {
            // -- specific status code (specified as a decimal value)
            int sc = StringTools.parseInt(eventCode,-1);
            if ((sc > 0) && (sc <= 0xFFFF)) {
                statusCode = sc;
            }
