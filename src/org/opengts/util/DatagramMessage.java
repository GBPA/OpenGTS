...
/**
*** A class for sending and recieving UDP datagram messages [CHECK]
**/
...
public class DatagramMessage
{
    ...
    **/
    public static void main(String argv[])
    {
        ...
        /* send data [UDP] */
        // -- bin/exeJava org.opengts.util.DatagramMessage -host=dcserver -port=1234 -recv -bindPort=12345 -send=0xFFFFFFFFF
        if (RTConfig.hasProperty(ARG_SEND)) {
            ...
            if (cmdRecv && (bindPort <= 0)) {
                Print.logError("'-recv' requires '-bindPort'");
                usage();
            }
            // --
            String dataStr = RTConfig.getString(ARG_SEND,"Hello World");
            Object sendObj[] = StringTools.split(dataStr,',');
            Payload p = Payload.CreatePayload(sendObj);
            byte send[] = p.getBytes();
            if (ListTools.isEmpty(send)) {
                Print.logError("Nothing to send");
                usage();
            }
            // --
            ...
            try {
                ...
                dgm.send(send);
                Print.logInfo("Sent [%s:%d]: 0x%s", host, port, StringTools.toHexString(send));
                if (cmdRecv) {
                    ...
                    if (sa instanceof InetSocketAddress) {
                        ...
                        Print.logInfo("Received from " + hostAddr + ":" + recvPort);
                        String h = "0x"+StringTools.toHexString(recv);
                        String s = StringTools.toStringValue(recv,'.');
                        Print.logInfo("Hex : " + h);
                        Print.logInfo("Text: " + s);
                    }
                }
            } catch (Throwable th) {
