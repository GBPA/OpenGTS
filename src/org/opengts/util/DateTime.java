...
//     -Added parsing of ISO_8601 formatted duration value "ParseDurationHours" [2.6.5-B21]
// ----------------------------------------------------------------------------
    ...
    implements Comparable<Object>, Cloneable
{
    ...
    public static final long     EPOCH_DAY_NUMBER           = 141427L;
    public static final int      EPOCH_YEAR                 = 1970;
    public static final int      EPOCH_MONTH1               = 1;
    public static final int      EPOCH_DAY                  = 1;

    public static final long     GREGORIAN_DAY_NUMBER       = 0L;
    public static final int      GREGORIAN_YEAR             = 1582;
    public static final int      GREGORIAN_MONTH1           = 10;
    public static final int      GREGORIAN_DAY              = 15;
    ...
    **/
    public enum DateStringFormat implements EnumTools.StringLocale, EnumTools.IntValue {
        ...
        MDY    ( 1, "mm/dd/yyyy" ),
        DMY    ( 2, "dd/mm/yyyy" ); // changed int-value to '2' [2.6.5-B19]
        // ---
        private int         vv = 0;
  ...
  //public static final int  MAY                = MAY;
    ...
    **/
        ...
        throws DateParseException
    {
        ...
        if (dateGrp.length == 1) {
            ...
            if ((dlen >= 19) && (dateStr.charAt(10) == 'T')) {
                // GPX format:
                //  YYYY-MM-DDThh:mm:ss
                ...
                String tz = (dlen > 19)? dateStr.substring(19) : "";    // Z, +00:00
    ...
    **/
    ...
    /**
    *** Parse ISO_8601 Duration values. <br>
    *** Reference: https://en.wikipedia.org/wiki/ISO_8601#Durations <br>
    *** Supports the following format: <br>
    ***    P[#Y][#M][#W][#D][T[#H][#M][#S]] <br>
    *** Where '#' is a string of 1 or more digits (a single decimal point 
    *** may also be specified to designate a fractional value. <br>
    *** Notes:<br>
    *** - If Year or Month values are specified, average values are used 
    ***   (ie. A year is 365.2425 days.  A month is 30.4375 days.)
    **/
    public static double ParseDurationHours(String durStr)
    {

        /* nothing to parse? */
        if (StringTools.isBlank(durStr)) {
            Print.logDebug("[WARN] Invalid duration format: "+durStr+" (blank)");
            return -1.0;
        }

        /* Starts With "P"? */
        if (!durStr.startsWith("P")) {
            Print.logDebug("[WARN] Invalid duration format: "+durStr+" (Missing leading 'P')");
            return -1.0;
        }

        /* parse elapsed time */
        double  hours   = 0.0;
        boolean timeInd = false;
        StringBuffer valSB = new StringBuffer();
        for (int s = 1; s < durStr.length(); s++) {
            char ch = Character.toUpperCase(durStr.charAt(s));

            /* value accumulator */
            if ((ch == '.') || (ch == ',')) {
                // -- accumulator: fractional value
                if (valSB.toString().indexOf(".") >= 0) {
                    Print.logDebug("[WARN] Invalid duration format: "+durStr+" (duplicate '.')");
                    return -1.0;
                }
                valSB.append(".");
                continue;
            ...
            /* switch to time indicator mode */
            if (ch == 'T') {
                // -- time indicator, H/M/S follows
                timeInd = true;
                if (!StringTools.isBlank(valSB)) {
                    // -- some formats may allow: "YYYY[-]MM[-]DD" (not supported here)
                    Print.logDebug("[WARN] Invalid duration format: "+durStr+" (value before 'T')");
                    return -1.0;
                }
                continue;
            }
            ...
            /* parse value */
            //Print.logDebug("Accumulator["+ch+"]: " + valSB);
            double val = StringTools.parseDouble(valSB.toString(),-1.0);
            if (val < 0.0) {
                Print.logDebug("[WARN] Invalid duration format: "+durStr+" (invalid value)");
                return -1.0;
            }
            valSB.setLength(0); // clear value accumulator
            ...
            if (timeInd) {
                ...
                switch (ch) {
                    ...
                    case 'M' : // minute (exact)
                        hours += val / 60.0;
                        break;
                    case 'S' : // seconds (exact)
                        hours += val / 3600.0;
                        break;
                    default : 
                        Print.logDebug("[WARN] Invalid duration format: "+durStr+" (unexpected '"+ch+"')");
                        return -1.0;
                }
            } else {
                // -- Y/M/W/D values
                switch (ch) {
                    case 'Y' : // year (approximate: considering leap-years)
                        hours += (val * 365.2425) * 24.0; // average days in year
                        break;
                    case 'M' : // month (average month days)
                        hours += (val * 30.4375) * 24.0; // average days in month
                        break;
                    case 'W' : // week (exact)
                        hours += (val * 7.0) * 24.0; // always 7 days
                        break;
                    case 'D' : // day (exact)
                        hours += (val ) * 24.0;
                        break;
        ...
        /* final error check */
        if (valSB.length() > 0) {
            // -- some formats may allow: "hh[:]mm[:]ss" (not supported here)
            Print.logDebug("[WARN] Invalid duration format: "+durStr+" (tailing value '"+valSB+"')");
            return -1.0;
        }
    ...
    **/
    public static long getDayNumberFromDate(int year, int month1, int day)
    {
        ...
        // -- JulianDay = DayNumber + 2299161;
        // -    DayNumber(2016,10,20) + 2299161 ==> 158521 + 2299161 ==> 2457682
        // -    MySQL: FROM_DAYS(DayNumber(2016,10,20) + 578101) ==> FROM_DAYS(158521 + 578101) ==> FROM_DAYS(736622) ==> "2016-10-20"
    }
    ...
    **/
    ...
    {
        if (dayNumber >= DateTime.GREGORIAN_DAY_NUMBER) {
            long N      = dayNumber + 578042L;
            long C      = ((N * 1000L) - 200L) / 36524250L;
            long N1     = N + C - (C / 4L);
            long Y1     = ((N1 * 1000L) - 200L) / 365250L;
            ...
            int  day    = DateTime.GREGORIAN_DAY;
            int  month1 = DateTime.GREGORIAN_MONTH1;
            int  year   = DateTime.GREGORIAN_YEAR;
            return new ParsedDateTime(tmz, year, month1, day);
        }
    }
    ...
    **/
    ...
    /**
    *** Constructor.  
    *** @param timeSec  The Epoch time in seconds (GMT)
    **/
    ...
    **/
    ...
    private static final String ARG_DURATION[]          = new String[]{ "dur", "duration" };
    ...
    private static final String ARG_XOR[]               = new String[]{ "xor" };
    private static final String ARG_EXP[]               = new String[]{ "exp" };
    ...
    **/
    public static void main(String argv[])
    {
        ...
        if (RTConfig.hasProperty(ARG_TIMEZONE_NAME)) {
            ...
            Print.sysPrintln("  Short : " + tz.getDisplayName(false, TimeZone.SHORT));
            Print.sysPrintln("  Long  : " + tz.getDisplayName(false, TimeZone.LONG));
            Print.sysPrintln("  ID    : " + tz.getID());
            System.exit(0);
        }
        ...
        if (RTConfig.getBoolean(ARG_TIMEZONE_LIST,false)) {
            ...
            Vector<TimeZone> tzGroup = new Vector<TimeZone>();
            for (int i = 0; i <= tzid.length; i++) {
                TimeZone tz = (i < tzid.length)? TimeZone.getTimeZone(tzid[i]) : null; // case sensitive
                String id = (tz != null)? tz.getID() : "****";   // ie. "US/Pacific"
                String groupID = (id.indexOf("/") >= 0)? id.substring(0,id.indexOf("/")) : id;
                if ((lastGroupID != null) && !lastGroupID.equals(groupID)) {
                    // -- start of new group
                    if (!ListTools.isEmpty(tzGroup)) {
                        // -- display last group
                        Print.sysPrintln("");
                        // -- TODO: sort by rawOffset
                        ListTools.sort(tzGroup, new Comparator<TimeZone>() {
                            public int compare(TimeZone tz1, TimeZone tz2) {
                                long r1 = tz1.getRawOffset();
                                long r2 = tz2.getRawOffset();
                                if (r1 == r2) {
                                    String n1 = tz1.getDisplayName(false,TimeZone.SHORT); // tz1.getID();
                                    String n2 = tz2.getDisplayName(false,TimeZone.SHORT); // tz2.getID();
                        ...
                        // -- list
                        for (TimeZone gtz : tzGroup) {
                            String gid          = gtz.getID();
                            String name         = gtz.getDisplayName();
                            String shortName    = gtz.getDisplayName(false, TimeZone.SHORT); // ie. "PST"
                            String shortNameDST = gtz.getDisplayName(true , TimeZone.SHORT); // ie. "PDT"
                            String longName     = gtz.getDisplayName(false, TimeZone.LONG);  // ie. "Pacific Standard Time"
                            long   rawOffsetMS  = gtz.getRawOffset();
                            long   rawOffsetSec = rawOffsetMS / 1000L;
                            String rawOffsetHMS = StringTools.formatElapsedSeconds(rawOffsetSec,StringTools.ELAPSED_FORMAT_HHMMSS);
                            String testName     = TimeZone.getTimeZone(shortName).getDisplayName(false, TimeZone.SHORT);
                            String gidStr       = StringTools.padRight(gid,' ',32);
                            String shortStr     = StringTools.padRight(shortName,' ',10);
                            String shortDSTStr  = StringTools.padRight(shortNameDST,' ',10);
                            String offsetStr    = StringTools.padLeft(rawOffsetHMS,' ',9);
                            Print.sysPrintln("ID=" +gidStr + ", SHORT=" + shortStr + "/" + shortDSTStr + ", Offset=" + offsetStr + ", LONG=" + longName); // + " [" + testName + "]");
                        }
                    }
                    tzGroup.clear();
                }
        ...
        /* time zones */
        TimeZone tz = DateTime.getDefaultTimeZone();
        if (RTConfig.hasProperty(ARG_TIMEZONE)) {
            String tzStr = RTConfig.getString(ARG_TIMEZONE, null);
            if (!StringTools.isBlank(tzStr)) {
                tz = DateTime.getTimeZone(tzStr, tz);  
                //Print.sysPrintln("Timezone   : " + tzStr + " ==> " + tz.getID());
            }
        }
        ...
        /* duration */
        if (RTConfig.hasProperty(ARG_DURATION)) {
            String durStr = RTConfig.getString(ARG_DURATION,"PT12H30M30S");
            double hours = ParseDurationHours(durStr);
            Print.sysPrintln("Duration Hours: " + hours);
            System.exit(0);
        }
        ...
        String timeHexDisp;
        {
            ...
            tsb.append(timeSec).append(":0x").append(StringTools.toHexString(timeSec,32));
            if (RTConfig.hasProperty(ARG_XOR)) {
                long xor = StringTools.parseHexLong(RTConfig.getString(ARG_XOR,null),0L);
                long timeXor = (timeSec ^ xor) & 0xFFFFFFFFL;
                tsb.append(" xor=0x").append(StringTools.toHexString(timeXor,32));
            }
            timeHexDisp = tsb.toString();
        }

        /* GMT date */
        DateTime gmtTimeDt = new DateTime(timeSec,gmtTZ);
        Print.sysPrintln("GMT time     : " + gmtTimeDt + " [" + timeHexDisp + "]");

        /* TimeZone date */
        DateTime tmzTimeDt = timeDT;
        Print.sysPrintln("TZ  time     : " + tmzTimeDt + " [name=" + tz.getDisplayName() + ", id=" + tz.getID() + "]");
