    ...
    extends Thread
{
    ...
    **/
    ...
    /**
    *** Opens the TCP socket connection
    ...
    **/
    public int socketReadByte()
        throws IOException
    {
        return this.socketReadByte(this.socket);
    }

    /**
    *** Reads a single byte from the specifed socket
    *** @param socket  The socket from which a byte is read
    *** @throws IOException if an error occured or the server has stopped
    **/
    protected int socketReadByte(Socket socket)
        throws IOException
    {
        if (socket == null) {
            return -1;
    ...
    **/
    ...
    {
        if (this.actionListeners != null) {
            for (Iterator<ActionListener> i = this.actionListeners.iterator(); i.hasNext();) {
