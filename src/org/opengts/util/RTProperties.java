            ...
            for (int i = 0; i < ds.length; i++) {
                ...
                if (d.startsWith(KEY_IF)) {
                    ...
                    if (ifVal != null) {
                        ...
                        if (ifVal.startsWith("*") && ifVal.endsWith("*")) {
                            // -- key is defined, compare contains
                            String v = ifVal.substring(1,ifVal.length()-1);
                            boolean eq = StringTools.containsIgnoreCase(pkVal,v);
                            inclDef = not? !eq : eq;
                        } else
                        if (ifVal.endsWith("*")) {
                            // -- key is defined, compare startsWith
                            String v = ifVal.substring(0,ifVal.length()-1);
                            boolean eq = StringTools.startsWithIgnoreCase(pkVal,v);
                            inclDef = not? !eq : eq;
                        } else
                        if (ifVal.startsWith("*")) {
                            // -- key is defined, compare endsWith
                            String v = ifVal.substring(1);
                            boolean eq = StringTools.endsWithIgnoreCase(pkVal,v);
                            inclDef = not? !eq : eq;
                        } else {
                            // -- key is defined, compare equals
