...
//     -Added "isValidBCD"
//  2017/02/18  Martin D. Flynn
//     -Added "readByte()"
//  2017/09/25  Martin D. Flynn
//     -Added "readInt(...)" with BigEndian parameter
// ----------------------------------------------------------------------------
...
public class Payload
{
    ...
    **/
    ...
    /**
    *** Creates a new Payload instance with the contents specified as arguments.
    *** The specified Objects may be one of the following types:
    ***     - byte[]
    ***     - String starting with "0x" (converted to hex byte array)
    ***     - String (written as bytes)
    ***     - Byte (1 byte)
    ***     - Short (2 bytes)
    ***     - Integer (4 bytes)
    ***     - Long (8 bytes)
    *** @return The resulting Payload instance (does not return null)
    **/
    public static Payload CreatePayload(Object... objs)
    {
        int size = DEFAULT_MAX_PAYLOAD_LENGTH;
        Payload p = new Payload(size);
        for (Object obj : objs) {
            if (obj instanceof byte[]) {
                p.writeBytes((byte[])obj);
            } else
            if ((obj instanceof String) && ((String)obj).startsWith("0x")) {
                p.writeBytes(StringTools.parseHex((String)obj,null));
            } else
            if (obj instanceof String) {
                p.writeBytes(((String)obj).getBytes());
            } else
            if (obj instanceof Byte) {
                p.writeBytes(Payload.encodeLong((Number)obj,1,true));
            } else
            if (obj instanceof Short) {
                p.writeBytes(Payload.encodeLong((Number)obj,2,true));
            } else
            if (obj instanceof Integer) {
                p.writeBytes(Payload.encodeLong((Number)obj,4,true));
            } else
            if (obj instanceof Long) {
                p.writeBytes(Payload.encodeLong((Number)obj,8,true));
            } else {
                // -- ignore unrecognized object
            }
        }
        return p;
    }
  ...
  //private ByteOrder   byteOrder       = ByteOrder.BIG_ENDIAN; // java.nio.ByteOrder
    ...
    **/
    public Payload(int maxPayloadLen, boolean bigEndian)
    {
        ...
        this.index   = 0; // start at index '0' for writing/reading
    ...
    **/
    public byte[] getBytes()
    {
        ...
        return this._getBytes(0, this.size);
    }
    ...
    *** @return The byte array currently in the payload (as-is)
    **/
    public byte[] getBytes(int maxLen)
    {
        // -- return the full payload, up to maxLen (regardless of the state of 'this.index')
        if (maxLen == 0) {
            // -- return zero bytes
            return new byte[0];
        } else
        if (maxLen < 0) {
            ...
            // -- up to maxLen bytes
            return this._getBytes(0, Math.min(maxLen,this.size));
        }
    }
    ...
    *** @return The byte read, or -1 if no bytes were available
    **/
    public int readByte()
    {
        byte b[] = this._readBytes(1, null/*msg*/, 1);
        return ((b != null) && (b.length > 0))? ((int)b[0] & 0xFF) : -1;
    }
    ...
    *** @return The byte read, or -1 if no bytes were available
    **/
    public int readByte(String msg)
    {
        byte b[] = this._readBytes(1, msg, 1);
        return ((b != null) && (b.length > 0))? ((int)b[0] & 0xFF) : -1;
    }
    ...
    **/
    ...
    /**
    *** Reverses the byte order of the specified 64-bit long.<br>
    *** IE: Convert Bit-Endian to Little-Endian, and visa-versa.
    *** @param value The Long value to reverse
    *** @return The reversed long
    **/
    public static long reverseByteOrder(long value)
    {
        return Payload.reverseByteOrder(value,8);
    }

    /**
    *** Reverses the byte order of the specified 32-bit integer.<br>
    *** IE: Convert Bit-Endian to Little-Endian, and visa-versa.
    ...
    **/
    ...
    {
        return (int)Payload.reverseByteOrder((long)value,4);
    }
    ...
    *** @return The decoded value, or the default value
    **/
    public int readInt(int length, int dft, boolean bigEndian)
    {
        return (int)this._readLong(length, (long)dft, bigEndian, 1, null/*msg*/);
    }
    ...
    *** @return The decoded value, or the default value
    **/
    public int readInt(int length, int dft, boolean bigEndian, String msg)
    {
        return (int)this._readLong(length, (long)dft, bigEndian, 1, msg);
    }
    ...
    **/
    public static byte[] encodeLong(long val, int len, boolean bigEndian)
    {
        ...
        Payload.encodeLong(data, 0, len, bigEndian, val);
        return data;
    }

    /**
    *** Encodes a <code>Number</code> instance into bytes
    *** @param val       The Number to encode
    *** @param len       The number of bytes to encode
    *** @param bigEndian True if the bytes are to be Big-Endian ordered, false if Little-Endian
    *** @return The encoded byte array
    **/
    public static byte[] encodeLong(Number num, int len, boolean bigEndian)
    {
        byte data[] = new byte[len];
        Payload.encodeLong(data, 0, len, bigEndian, (num != null)? num.longValue() : 0L);
        return data;
    }
    ...
    **/
    ...
    {
        String valStr = (signed?"(long) ":"(ulong) ") + String.valueOf(val) + (bigEndian?"":" (LE)");
        this._printDebug(frame + 1, maxLen, valStr, msg);
    }
