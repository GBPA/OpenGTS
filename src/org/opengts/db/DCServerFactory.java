...
//     -Fixed "_loadDeviceByPrefixedModemID" to check for prefix type "*" [2.6.2-B19]
// ----------------------------------------------------------------------------
...
public class DCServerFactory
{
    ...
    **/
    ...
    public  static final String  PREFIX_BLANK        = "<blank>";
    public  static final String  PREFIX_ANY          = "*";

    private static final String  BLANK_PREFIX_LIST[] = {
        PREFIX_BLANK,
        PREFIX_ANY,
        ""
    };
    ...
    /** 
    *** Returns true if the specified value represents a blank prefix
    **/
    public static boolean IsBlankPrefix(String pfx)
    {
        if (StringTools.isBlank(pfx)) {
            return true;
        } else {
            for (String bpf : BLANK_PREFIX_LIST) {
                if (bpf.equalsIgnoreCase(pfx)) {
                    return true;
                }
            }
            return false;
    ...
    /** 
    *** Assembles a UniqueID based on the specified prefix and mobileID
    **/
    public static String CreateUniqueID(String pfx, String mobileID)
    {
        return DCServerFactory.IsBlankPrefix(pfx)? mobileID : (pfx + mobileID);
    }
    ...
    public static final String  TK10X_NAME                      = "tk10x";          // [31272] 3X270/3x272
    ...
    **/
    ...
    public static final String  CMDARG_ARG                      = DCServerConfig.DEFAULT_ARG_NAME; // "arg"
    ...
    public static final String  RESPONSE_SMS                    = "sms";
    ...
    **/
    public enum ResultCode implements EnumTools.StringLocale, EnumTools.StringValue {
        ...
        SUCCESS_UDP      ("OK001",I18N.getString(DCServerFactory.class,"DCServerFactory.result.successfulUDP"   ,"Successful UDP")),
        SUCCESS_TCP      ("OK002",I18N.getString(DCServerFactory.class,"DCServerFactory.result.successfulTCP"   ,"Successful TCP")),
        SUCCESS_SMS      ("OK003",I18N.getString(DCServerFactory.class,"DCServerFactory.result.successfulSMS"   ,"Successful SMS")),
        COMMAND_QUEUED   ("OK010",I18N.getString(DCServerFactory.class,"DCServerFactory.result.commandQueued"   ,"Command Queued")),
        ...
        public boolean isSuccess()            { return this.cc.startsWith("OK"); }
    };
    ...
    **/
    ...
    /**
    *** Returns true if the specified ResultCode represents a success value
    *** @param dft   The ResultCode
    *** @return True if successful, false otherwise
    **/
    public static boolean IsResultCodeSuccess(ResultCode code)
    {
        return ((code != null) && code.isSuccess())? true : false;
    }
    ...
    private static final String TAG_IfDevice            = "IfDevice";
  ...
  //private static final String ATTR_portOffset         = "portOffset";
    ...
    private static final String ATTR_compare            = "compare";
                        ...
                        for (int eci = 0; eci < codeList.getLength(); eci++) {
                            ...
                            if (codeNode instanceof Element) {
                                ...
                                if (!StringTools.isBlank(keyStr)) {
                                    ...
                                    if (valStr.equalsIgnoreCase("ignore")) {
                                ...
                                if (cmdEnable || !cmdDiscard) {
                                    ...
                                    for (int s = 0; s < cmdSubNodes.getLength(); s++) {
                                        ...
                                        if (cmdNodeName.equalsIgnoreCase(TAG_Type)) {
                                            ...
                                            cmdTypes = StringTools.parseStringArray(typ,','); // may be empty 
                                        } else
                                        ...
                                        if (cmdNodeName.equalsIgnoreCase(TAG_IfDevice)) {
                                            ifField   = XMLTools.getAttribute(cmdSubElem,ATTR_field,"",false);
                                            ifCompare = XMLTools.getAttribute(cmdSubElem,ATTR_compare,"",false); // EQ|NE|GE|LE|CONTAINS|INLIST
                                            ifValue   = XMLTools.getNodeText(cmdSubElem,"");
                                        } else
    ...
    public static boolean isCommandResultOK(RTProperties r)
    {
        if (r != null) {
            ...
            if (StringTools.isBlank(rid)) {
                ...
                return DCServerFactory.isCommandResultOK(rsID);
            }
        } else {
            return false;
        }
    ...
    **/
    ...
    /**
    *** Gets the CommandProtocol
    **/
    public static DCServerConfig.CommandProtocol getCommandProtocol(RTProperties r)
    {
        if (r == null) {
            return null;
        }
        ResultCode rid = DCServerFactory.getCommandResultCode(r);
        switch (rid) {
            case SUCCESS_UDP : 
                return DCServerConfig.CommandProtocol.UDP;
            case SUCCESS_TCP : 
                return DCServerConfig.CommandProtocol.TCP;
            case SUCCESS_SMS : 
                return DCServerConfig.CommandProtocol.SMS;
            case SUCCESS     :
                return DCServerConfig.getCommandProtocol(r.getString(DCServerFactory.RESPONSE_PROTOCOL,null)); // may return null
        }
        return null;
    }
    ...
    **/
    private static DCServerFactory.ResultCode _SendSMSCommand(Device device, String commandStr)
    {
        ...
        /* get handler */
        String smsHandler = account.getSmsProperties(null).getGatewayName();
    ...
    *** @return The ResultCode
    **/
    public static DCServerFactory.ResultCode SendSMSCommand(Device device, String commandStr)
    {
        ...
        /* send SMS command */
        DCServerFactory.ResultCode resp = DCServerFactory._SendSMSCommand(device, commandStr);
        DCServerConfig.CommandProtocol cmdProto = DCServerConfig.CommandProtocol.SMS;
        ...
        if ((device != null) && DCServerFactory.isCommandResultOK(resp)) {
          ...
          //if (!ListTools.isEmpty(cmdArgs)) { cmdStr += "(" + StringTools.join(cmdArgs,",") + ")"; }
            Audit.deviceCommand(acctID, userID, devID, ipAddr, cmdStr, cmdProto); // SMS command
        } else {
            Print.logInfo(dcsCtx + "SMS Response: " + resp);
        }
    ...
    **/
        ...
        String cmdType, String cmdName, String cmdArgs[])
    {
        ...
        if (cmdPort <= 0) {
            if (internal) {
                Print.logError("[%s] Command not supported via SMS: %s/%s", serverName, cmdType, cmdName);
                return null;
            }
            DCServerConfig.Command command = dcs.getCommand(cmdName);
            if (command == null) {
                Print.logError("[%s] Command not supported: %s/%s", serverName, cmdType, cmdName);
                return null;
            } else
            if (!command.isCommandProtocolSMS()) {
                Print.logError("[%s] Command port not supported: %s/%s", serverName, cmdType, cmdName);
                return null;
            }
            // -- send command via SMS (SmsOutboudGatweay)
            String commandStr = command.getCommandString(device, cmdArgs);
            DCServerFactory.ResultCode result = DCServerFactory._SendSMSCommand(device, commandStr);
    ...
    **/
        ...
        String cmdType, String cmdName, String cmdArgs[])
    {
        ...
        DCServerConfig.CommandProtocol cmdProto = DCServerFactory.getCommandProtocol(resp);
    ...
    /** (NOTE: may not be supported in this version)
    *** Gets the current active TCP session info for this device.
    *** Returns null if this device is not currently connected to a DCS using TCP
    **/
    public static RTProperties getActiveSessionInfo(Device device)
    {
        return null;
    }
    ...
    *** @return The Device record
    **/
    public static Device _loadDeviceByPrefixedModemID_throw(String prefix[], String modemID)
        throws DBException, Throwable
    {
        return DCServerFactory._loadDeviceByPrefixedModemID_throw(prefix, modemID, 
    ...
    **/
    ...
    {
        try {
            return _loadDeviceByPrefixedModemID_throw(prefix, modemID, 
                saveUnassigned, serverID, ipAddress, isDuplex, geoPoint);
        } catch (DBException dbe) {
        ...
        throws DBException, Throwable
    {
        ...
        try {
            ...
            if (ListTools.isEmpty(prefix)) {
                ...
                uniqueID = DCServerFactory.CreateUniqueID(prefix[0], modemID);
                for (int u = 0; u < prefix.length; u++) {
                    String pfxID = DCServerFactory.CreateUniqueID(prefix[u], modemID); // [2.6.2-B19]
            ...
            if (device == null) {
                ...
                if ((acct != null) && acct.okToAutoAddDevice()) { // getAutoAddDevices()
                    // -- Create a 'Device'
                    try {
            ...
            String  devID  = device.getDeviceID();
            if (acct == null) {
                Print.logWarn("Account not found: " + acctID + "/" + devID + " [" + uniqueID + "]");
                return null;
            } else
            if (!acct.isActive()) {
                Print.logWarn("Account is inactive: " + acctID + "/" + devID + " [" + uniqueID + "]");
                return null;
            } else
            if (acct.isExpired()) {
                Print.logWarn("Account is expired: " + acctID + "/" + devID + " [" + uniqueID + "]");
                return null;
            } else
            if (!device.isDCSActive()) {
                Print.logWarn("Account/Device is inactive: " + acctID + "/" + devID + " [" + uniqueID + "]");
                return null;
            }
            ...
            Print.logError("Exception getting Device: " + uniqueID + " [" + th + "]");
            throw th; // rethrow
        }
    ...
    **/
    public static String getUniquePrefixString(String pfx[])
    {
        if (ListTools.isEmpty(pfx)) {
            return DCServerFactory.PREFIX_BLANK;
        } else {
            StringBuffer sb = new StringBuffer();
            String list[] = new String[pfx.length];
            for (int i = 0; i < pfx.length; i++) {
                if (sb.length() > 0) { sb.append(", "); }
                if (DCServerFactory.IsBlankPrefix(pfx[i])) {
                    sb.append(DCServerFactory.PREFIX_BLANK);
                } else {
    ...
    public static final String  CFG_uniquePrefix                = ".uniquePrefix";               // String[]
    public static final String  CFG_uniqueIdPrefix              = ".uniqueIdPrefix";             // String[]
    ...
    public static final String  CFG_minimumMovedMetersAge       = ".minimumMovedMetersAge";      // long
    ...
    public static final String  CFG_statusLocationInMotion      = ".statusLocationInMotion";     // boolean
    ...
    **/
    ...
    /**
    *** Return an array of "Minimum Moved Meters Age" property names
    *** @param name  The server name
    *** @return An array of "Minimum Moved Meters Age" names
    **/
    public static String[] CONFIG_minimumMovedMetersAge(String name)
    {
        return new String[] { 
            PROP_DCServer_ + name + CFG_minimumMovedMetersAge,
            name + CFG_minimumMovedMetersAge
        };
    }
