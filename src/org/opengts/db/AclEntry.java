...
//     -Remove "${dcsDesc}" from DCS ACL description if unable to replace.
// ----------------------------------------------------------------------------
        ...
        if (this.aclDesc == null) {
            ...
            if (p >= 0) {
                if (this.aclName.startsWith(ACL_DCS_)) {
                    // -- replace the key "${dcsDesc}" with the custom DCS "server" description
                    ...
                    // -- just remove the "${dcsDesc}" string
                    aclDesc = aclDesc.substring(p+k_dcsDesc.length()).trim();
                }
            }
            return aclDesc;
        }
