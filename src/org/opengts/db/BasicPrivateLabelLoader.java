...
public class BasicPrivateLabelLoader
{
    ...
    public  static final String TAG_RouteProvider               = "RouteProvider";
  ...
  //public  static final String TAG_EMailAddress                = "EMailAddress";
    ...
    public  static final String TAG_OmitColumn                  = "OmitColumn";
  ...
  //public  static final String TAG_Description                 = "Description";
    ...
    public  static final String ATTR_mobileDomain               = "mobileDomain";
    ...
    **/
    ...
    {
        String actv = StringTools.trim(active);
        if (StringTools.isBlank(actv)) {
            // -- blank is false
            return false;
        } else
        if (StringTools.isBoolean(actv,true)) {
            // -- explicit boolean value
            return StringTools.parseBoolean(actv,false);
        } else {
            // -- equals 'name' attribute
            return actv.equalsIgnoreCase(name);
        }
    }
    ...
    *** @return The number of domains loaded
    **/
    protected int _loadXML(File xmlFile, String xmlIncl,
        RTProperties dftProps, RTProperties ovrProps, 
        OrderedMap<String,PushpinIcon> dftPushpinMap,
        OrderedMap<String,String> dftLegend,
        boolean ignoreDuplicates)
    {
        printDebug("Loading PrivateLabel xml file: " + xmlFile);
        printDebug("ContextPath: " + RTConfig.getContextPath());
        ...
        if (!privLabels.getTagName().equalsIgnoreCase(TAG_PrivateLabels)) {
            printError("Missing tag ["+xmlFile+"]: "+TAG_PrivateLabels);
            this._setHasParsingErrors(xmlFile);
            return 0;
        }
        ...
        globalProps.setDebugName("LoadXMLGLobal");
        ...
        //globalProps.printProperties("Global Properties ("+xmlIncl+"): " + xmlFile);

        /* PushPins: parse default <PushPins> (if present) */
        boolean isGlobalPushpins = (dftPushpinMap == null);
        dftPushpinMap = new OrderedMap<String,PushpinIcon>(dftPushpinMap);
        ...
        for (int p = 0; p < ppNodes.getLength(); p++) {
            ...
            OrderedMap<String,PushpinIcon> ppMap = this.parseTAG_Pushpins(xmlFile, null, ppn, null, isGlobalPushpins);
        ...
        if ((includeList.getLength() > 0) && (DomainIncludeRecursionLevel > 0)) {
            ...
            for (int i = 0; i < includeList.getLength(); i++) {
                ...
                if (StringTools.isBlank(includeFile)) {
                    printError("Include 'file' not specified ["+xmlFile+"]: "+inclName);
                    this._setHasParsingErrors(xmlFile);
                    continue;
                }
                ...
                inclProps.setDebugName("IncludeOverride");
                ...
                for (int e = 0; e < propList.getLength(); e++) {
                    ...
                    if (propName.equalsIgnoreCase(TAG_LogMessage)) {
                        ...
                        printError("Invalid tag name ["+xmlFile+", include '"+inclName+"']: " + propName + " [expecting "+TAG_Property+"]");
                        this._setHasParsingErrors(xmlFile);
                    }
                }
                ...
                //inclProps.printProperties("Include properties: " + inclName);
                ...
                if ((inclFile != null) && inclFile.isFile()) {
                    try {
                        ...
                        if (inclFilePath.equals(xmlFile.getCanonicalPath())) {
                            ...
                            int cnt = this._loadXML(inclFile, inclName,
                        ...
                        printError("Error while including file ["+xmlFile+"]: " + inclFile);
    ...
    **/
    protected OrderedSet<String> parseTag_Timezones(Element elemNode)
    {
        ...
        for (int tzl = 0; tzl < timeZonesNodes.getLength(); tzl++) {
            ...
            // -- load from file?
            String tmzFileS = XMLTools.getAttribute(tmzsTag,ATTR_file,null,false);
            if (!StringTools.isBlank(tmzFileS)) {
                File cfgFile = RTConfig.getLoadedConfigFile();
                File tmzFile = new File(cfgFile.getParentFile(), tmzFileS); // "WEB-INF/timezones.conf"
                if (tmzFile.isFile()) {
                    // -- load
                }
            }
    ...
    **/
        ...
        boolean ignoreDuplicates)
    {
        ...
        if (overrideProps != null) {
            overrideProps.setDebugName("PrivateLabelOverride");
            RTConfig.pushThreadProperties(overrideProps);
        ...
        String  mobileDomain   = XMLTools.getAttribute(       domain, ATTR_mobileDomain  , null , true );
        ...
        if (StringTools.isBlank(domainName)) {
            printError("Domain 'name' attribute not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
        }

        /* i18n RTProperties Strings */
        RTProperties i18nStr = new RTProperties();
        i18nStr.setDebugName("I18NString");
        RTConfig.pushThreadProperties(i18nStr);
        ...
        pl.setMobileDomainName(mobileDomain); // name of Mobile Domain (if any)
        ...
        for (int c = 0; c < attrList.getLength(); c++) {
            ...
            if (attrName.equalsIgnoreCase(TAG_Author)) {
                String author = XMLTools.getNodeText(attrElem," ",true);
                //Print.logInfo("Author("+xmlFile+"): " + author);
                pl.setAuthor(author);
            } else
            ...
            if (attrName.equalsIgnoreCase(TAG_RouteProvider)) {
                this.parseTag_RouteProvider(xmlFile, pl, attrElem);
            } else
            ...
            if (attrName.equalsIgnoreCase(TAG_Pushpins)) {
                OrderedMap<String,PushpinIcon> ppMap = this.parseTAG_Pushpins(xmlFile, pl, attrElem, null, false);
                if (!ListTools.isEmpty(ppMap)) {
                    dftPushpinMap.putAll(ppMap);
                }
            } else
            ...
            if (attrName.equalsIgnoreCase(TAG_LogMessage)) {
                ...
                printError("Invalid/Unrecognized tag name ["+xmlFile+"]: " + attrName);
                this._setHasParsingErrors(xmlFile);
            }
        ...
        /* pop I18N properties */
        RTConfig.popThreadProperties(i18nStr);

        /* pop override properties */
        if (popOverrideProps) {
            RTConfig.popThreadProperties(overrideProps);
        }
    ...
    protected void parseTag_I18N(File xmlFile, String i18nPkgName, BasicPrivateLabel pl, Element attrElem, RTProperties i18nStr)
    {
        ...
        for (int e = 0; e < propList.getLength(); e++) {
            ...
            if (propName.equalsIgnoreCase(TAG_String)) {
                ...
                printError("Invalid tag name ["+xmlFile+"]: " + propName + " [expecting "+TAG_String+"]");
                this._setHasParsingErrors(xmlFile);
            }
        ...
        String keyPrefix, String rtPrefix, boolean defaultRTPOverride)
    {
        ...
        for (int e = 0; e < propList.getLength(); e++) {
            ...
            if (propName.equalsIgnoreCase(TAG_LogMessage)) {
                ...
                printError("Invalid tag name ["+xmlFile+"]: " + propName + " [expecting "+TAG_Property+"]");
                this._setHasParsingErrors(xmlFile);
            }
    ...
    protected void parseTag_EMailAddresses(File xmlFile, BasicPrivateLabel pl, Element listAttrElem)
    {
        ...
        for (int e = 0; e < emailAttrList.getLength(); e++) {
            ...
            if (emailAttrName.equalsIgnoreCase(TAG_EMailAddress)) {
                ...
                if (!StringTools.isBlank(emailAddr)) {
                    ...
                    if (this.isValidEMailAddress(emailAddr)) {
                        ...
                        printError("Invalid EMail address ["+xmlFile+"]: '"+emailAddr+"'");
                        this._setHasParsingErrors(xmlFile);
                    }
                }
            } else
            if (emailAttrName.equalsIgnoreCase(TAG_LogMessage)) {
                ...
                printError("Invalid tag name ["+xmlFile+"]: " + emailAttrName + " [expecting "+TAG_EMailAddress+"]");
                this._setHasParsingErrors(xmlFile);
            }
        ...
        for (int c = 0; c < scAttrList.getLength(); c++) {
            ...
            if (scAttrName.equalsIgnoreCase(TAG_StatusCode)) {
                ...
                if (code > 0) {
                    ...
                    printError("Code missing or Invalid ["+xmlFile+"]: " + codeStr);
                    this._setHasParsingErrors(xmlFile);
                }
            } else
            if (scAttrName.equalsIgnoreCase(TAG_LogMessage)) {
                ...
                printError("Invalid tag name ["+xmlFile+"]: " + scAttrName + " [expecting "+TAG_StatusCode+"]");
                this._setHasParsingErrors(xmlFile);
            }
    ...
    **/
    protected void parseTag_PasswordHandler(File xmlFile, BasicPrivateLabel pl, Element phAttrElem)
    {
        ...
        if (StringTools.isBlank(phClassName)) {
            printError("PasswordHandler 'class' not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
            return;
        }
        ...
        rtProps.setDebugName("PasswordHandler");
        ...
        for (int c = 0; c < propAttrList.getLength(); c++) {
            ...
            if (attrName.equalsIgnoreCase(TAG_Property)) { // "PasswordHandler" sub
                ...
                if (!StringTools.isBlank(key)) {
                    ...
                    if (!StringTools.isBlank(rtPropPrefix)) { // PasswordHandler
                        // -- override value
                        for (;/*single-pass-loop*/;) {
                            String v = null;
                            // -- "Domain.PasswordHandler.PHNAME.KEY"
                            if (!StringTools.isBlank(phName)) {
                                v = RTConfig.getString(rtPropPrefix + phName + "." + rtKey, null);
                                if (v != null) {
                                    val = v;
                                    break;
                                }
                            }
                            // -- "Domain.PasswordHandler.KEY"
                            v = RTConfig.getString(rtPropPrefix + rtKey, null);
                            if (v != null) {
                                val = v;
                                break;
                            }
                            // -- value not overridden (left as-is)
                            break; // exit SPL
                        }
                    }
            ...
            if (attrName.equalsIgnoreCase(TAG_LogMessage)) {
                ...
                printError("Invalid tag name ["+xmlFile+"]: " + attrName + " [expecting "+TAG_Property+"]");
                this._setHasParsingErrors(xmlFile);
            }
        ...
        try {
            ...
            printError("PasswordHandler class not found ["+xmlFile+"]: " + phClassName);
            this._setHasParsingErrors(xmlFile);
            return;
        } catch (Throwable t) { // ClassNotFoundException, ClassCastException, etc.
            printError("PasswordHandler creation error ["+xmlFile+"]: " + phClassName + " [" + t);
            this._setHasParsingErrors(xmlFile);
            return;
        } 
    ...
    **/
    protected void parseTag_ReverseGeocodeProvider(File xmlFile, BasicPrivateLabel pl, Element rgpAttrElem)
    {
        ...
        if (StringTools.isBlank(rpName)) {
            printError("ReverseGeocodeProvider 'name' not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
            return;
        }
        ...
        if (StringTools.isBlank(rpClassName)) {
            if (active) {
                printError("ReverseGeocodeProvider 'class' not specified ["+xmlFile+"].");
                this._setHasParsingErrors(xmlFile);
            }
            return;
        }
        ...
        RTProperties rtProps = new RTProperties();
        rtProps.setDebugName("ReverseGeocodeProvider");
        ...
        for (int c = 0; c < propAttrList.getLength(); c++) {
            ...
            if (attrName.equalsIgnoreCase(TAG_Property)) { // "ReverseGeocodeProvider" sub
                ...
                if (!StringTools.isBlank(key)) {
                    boolean valTrim = XMLTools.getAttributeBoolean(attrElem,ATTR_trim,true,false);
                    String val = XMLTools.getNodeText(attrElem, (valTrim?"":null), true);
                    if (!StringTools.isBlank(rtPropPrefix)) { // ReverseGeocodeProvider
                        // -- override value
                        for (;/*single-pass-loop*/;) { // single-pass loop
                            String v = null;
                            // -- "Domain.ReverseGeocodeProvider.RGPNAME.KEY"
                            // -   eg. "Domain.ReverseGeocodeProvider.googleV3.failoverQuiet"
                            if (!StringTools.isBlank(rpName)) {
                                v = RTConfig.getString(rtPropPrefix + rpName + "." + rtKey, null);
                                if (v != null) {
                                    val = v;
                                    break;
                                }
                            ...
                            // -   eg. "Domain.ReverseGeocodeProvider.failoverQuiet"
                            {
                                v = RTConfig.getString(rtPropPrefix + rtKey, null);
                                if (v != null) {
                                    val = v;
        ...
        if (!StringTools.isBlank(failover)) {
            ...
            if (rgp != null) {
                ...
                printError("ReverseGeocodeProvider failover not defined ["+xmlFile+"]: " + failover);
                this._setHasParsingErrors(xmlFile);
                failover = null;
            }
        }
        ...
        try {
            ...
            if (active) {
                printError("ReverseGeocodeProvider class not found ["+xmlFile+"]: " + rpClassName);
                this._setHasParsingErrors(xmlFile);
            }
            return;
        } catch (Throwable t) { // ClassNotFoundException, ClassCastException, etc.
            if (active) {
                printError("ReverseGeocodeProvider creation error ["+xmlFile+"]: " + rpClassName + " [" + t);
                this._setHasParsingErrors(xmlFile);
            }
            return;
        }
        ...
        if (active && useAsGeocoder) {
            if (rgp instanceof GeocodeProvider) {
                ...
                printError("'geocode' specified, and ReverseGeocodeProvider is not a GeocodeProvider ["+xmlFile+"]: " + rpName);
                this._setHasParsingErrors(xmlFile);
            }
        } else {
            // skip
    ...
    **/
    protected void parseTag_GeocodeProvider(File xmlFile, BasicPrivateLabel pl, Element gpAttrElem)
    {
        ...
        if (StringTools.isBlank(gpName)) {
            printError("GeocodeProvider 'name' not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
            return;
        }
        ...
        if (StringTools.isBlank(gpClassName)) {
            if (active) {
                printError("GeocodeProvider 'class' not specified ["+xmlFile+"].");
                this._setHasParsingErrors(xmlFile);
            }
            return;
        }
        ...
        RTProperties rtProps = new RTProperties();
        rtProps.setDebugName("GeocodeProvider");
        ...
        for (int c = 0; c < propAttrList.getLength(); c++) {
            ...
            if (attrName.equalsIgnoreCase(TAG_Property)) { // "GeocodeProvider" sub
                String key = this._adjustPropertyKey(xmlFile,XMLTools.getAttribute(attrElem,ATTR_key,null,false));
                String rtKey = StringTools.blankDefault(XMLTools.getAttribute(attrElem,ATTR_rtKey,null,false),key);
                if (!StringTools.isBlank(key)) { // TODO: should be 'rtKey'?
                    boolean valTrim = XMLTools.getAttributeBoolean(attrElem,ATTR_trim,true,false);
                    String val = XMLTools.getNodeText(attrElem, (valTrim?"":null), true);
                    if (!StringTools.isBlank(rtPropPrefix)) { // GeocodeProvider
                        // -- override value
                        for (;/*single-pass-loop*/;) {
                            String v = null;
                            // -- check "Domain.GeocodeProvider.GPNAME.KEY"
                            if (!StringTools.isBlank(gpName)) {
                                v = RTConfig.getString(rtPropPrefix + gpName + "." + rtKey, null);
                                if (v != null) {
                                    val = v;
                                    break;
                                }
                            }
                            // -- check "Domain.GeocodeProvider.KEY"
                            v = RTConfig.getString(rtPropPrefix + rtKey, null);
                            if (v != null) {
                                val = v;
                                break;
        ...
        try {
            ...
            if (active) {
                printError("GeocodeProvider class not found ["+xmlFile+"]: " + gpClassName);
                this._setHasParsingErrors(xmlFile);
            }
            return;
        } catch (Throwable t) { // ClassNotFoundException, ClassCastException, etc.
            if (active) {
                printError("GeocodeProvider creation error ["+xmlFile+"]: " + gpClassName + " [" + t);
                this._setHasParsingErrors(xmlFile);
            }
            return;
        }
    ...
    **/
    protected void parseTag_MobileLocationProvider(File xmlFile, BasicPrivateLabel pl, Element mpAttrElem)
    {
        ...
        if (StringTools.isBlank(mpName)) {
            printError("MobileLocationProvider 'name' not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
            return;
        }
        ...
        if (StringTools.isBlank(mpClassName)) {
            printError("MobileLocationProvider 'class' not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
            return;
        }
        ...
        /* properties */
        RTProperties rtProps = new RTProperties();
        rtProps.setDebugName("MobileLocationProvider");
        ...
        try {
            ...
            printError("MobileLocationProvider class not found ["+xmlFile+"]: " + mpClassName);
            this._setHasParsingErrors(xmlFile);
            return;
        } catch (Throwable t) { // ClassNotFoundException, ClassCastException, etc.
            printError("MobileLocationProvider creation error ["+xmlFile+"]: " + mpClassName + " [" + t);
            this._setHasParsingErrors(xmlFile);
            return;
        }
    ...
    /**
    *** Parse 'RouteProvider' tag
    *** @param xmlFile      The current XML file being processed
    *** @param pl           The BasicPrivateLabel instance for the current 'Domain'
    *** @param rpAttrElem   The 'RouteProvider' node
    **/
    protected void parseTag_RouteProvider(File xmlFile, BasicPrivateLabel pl, Element rpAttrElem)
    {

        /* name */
        String rpName = XMLTools.getAttribute(rpAttrElem, ATTR_name, null, false);
        if (StringTools.isBlank(rpName)) {
            printError("RouteProvider 'name' not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
            return;
        }

        /* active? */
        String active = XMLTools.getAttribute(rpAttrElem, ATTR_active, null, true);
        if (!BasicPrivateLabelLoader._isAttributeActive(active,rpName)) {
            // -- inactive, quietly ignore
            return;
        }

        /* RouteProvider class name */
        String rpClassName = XMLTools.getAttribute(rpAttrElem, ATTR_class, null, false);
        if (StringTools.isBlank(rpClassName)) {
            printError("RouteProvider 'class' not specified ["+xmlFile+"].");
            this._setHasParsingErrors(xmlFile);
            return;
        }

        /* properties */
        String rtPropPrefix = XMLTools.getAttribute(rpAttrElem,ATTR_rtPropPrefix,null,true);
        RTProperties rtProps = new RTProperties();
        rtProps.setDebugName("RouteProvider");
        NodeList propAttrList = rpAttrElem.getChildNodes();
        for (int c = 0; c < propAttrList.getLength(); c++) {
            ...
            String attrName = attrNode.getNodeName();
            Element attrElem = (Element)attrNode;
            if (attrName.equalsIgnoreCase(TAG_Property)) { // "RouteProvider" sub
                String key = this._adjustPropertyKey(xmlFile,XMLTools.getAttribute(attrElem,ATTR_key,null,false));
                String rtKey = StringTools.blankDefault(XMLTools.getAttribute(attrElem,ATTR_rtKey,null,false),key);
                if (!StringTools.isBlank(key)) { // TODO: should be 'rtKey'?
                    boolean valTrim = XMLTools.getAttributeBoolean(attrElem,ATTR_trim,true,false);
                    String val = XMLTools.getNodeText(attrElem, (valTrim?"":null), true);
                    if (!StringTools.isBlank(rtPropPrefix)) { // RouteProvider
                        // -- override value
                        for (;/*single-pass-loop*/;) {
                            String v = null;
                            // -- check "Domain.RouteProvider.RPNAME.KEY"
                            if (!StringTools.isBlank(rpName)) {
                                v = RTConfig.getString(rtPropPrefix + rpName + "." + rtKey, null);
                                if (v != null) {
                                    val = v;
                            ...
                            // -- check "Domain.RouteProvider.KEY"
                            v = RTConfig.getString(rtPropPrefix + rtKey, null);
                            if (v != null) {
                                val = v;
                                break;
        ...
        /* create instance of RouteProvider */
        String rpKey = XMLTools.getAttribute(rpAttrElem,ATTR_key,null,true); // authorization key
        RouteProvider rp = null;
        try {
            Class<?> rpClass = Class.forName(rpClassName);  // ClassNotFoundException
            MethodAction ma = new MethodAction(rpClass, String.class, String.class, RTProperties.class);
            rp = (RouteProvider)ma.invoke(rpName, rpKey, rtProps);
        } catch (ClassNotFoundException cnfe) {
            printError("RouteProvider class not found ["+xmlFile+"]: " + rpClassName);
            this._setHasParsingErrors(xmlFile);
            return;
        } catch (Throwable t) { // ClassNotFoundException, ClassCastException, etc.
            printError("RouteProvider creation error ["+xmlFile+"]: " + rpClassName + " [" + t);
            this._setHasParsingErrors(xmlFile);
            return;
        }
        pl.addRouteProvider(rp);
        ...
        if (v.length == 1) {
            ...
            for (int i = 0; i < v.length; i++) {
                ...
                if (lastValue.getIntValue() == acc.getIntValue()) {
                    ...
                    printError("Invalid AccessLevel list specified ["+xmlFile+"]: " + strList);
                    this._setHasParsingErrors(xmlFile);
                }
                lastValue = acc;
            }
    ...
    **/
    ...
    protected void parseTag_EventNotificationEMail(File xmlFile, String i18nPackage, BasicPrivateLabel pl, Element attrElem)
    {
        ...
        if (!this.isValidEMailAddress(emailFrom)) {
            printError("Invalid EMail address ["+xmlFile+"]: "+emailFrom);
            this._setHasParsingErrors(xmlFile);
        }
    ...
    *** @param isGlobalPushpins  True if parsing globally defined pushpins
    *** @return 'null' (must be overridden to change behavior)
    **/
    protected OrderedMap<String,PushpinIcon> parseTAG_Pushpins(File xmlFile, BasicPrivateLabel pl, Element attrElem,
        OrderedMap<String,PushpinIcon> dftPushpinMap, boolean isGlobalPushpins)
    {
        // override
        return null;
    }
    ...
    **/
    public static BasicPrivateLabel getPrivateLabelForURL(URL url)
    {
        ...
        /* get BasicPrivateLabel from url */
        if (url != null) {
    ...
    *** @param rtnDefault  True to return default if name not found, otherwise return null
    *** @return The BasicPrivateLabel instance
    **/
    public static BasicPrivateLabel getPrivateLabel(String name, boolean rtnDefault)
    {
        BasicPrivateLabelLoader bpll = BasicPrivateLabelLoader._getInstance();
        return rtnDefault? bpll._getPrivateLabel(name) : bpll._getPrivateLabel(name,null);
    }
    ...
    **/
    ...
    {
        return BasicPrivateLabelLoader.getPrivateLabel(name,true);
    }
    ...
    **/
    ...
    {
        return BasicPrivateLabelLoader.getPrivateLabel(null,true);
    }
        ...
        if (privLabel != null) {
            ...
            for (String hostAlias : tempPLMap.keySet()) {
                if (this.privateLabelMap.containsKey(hostAlias)) {
                    BasicPrivateLabel dupPL = this.privateLabelMap.get(hostAlias);
                    if (ignoreDuplicates) {
                        // -- warning only
                        printWarn( "Domain Host/Alias already defined: [" + xmlFile + " : " + privLabel.getName() + "] " + hostAlias + " ==> " + dupPL.getName());
                        this._setHasParsingWarnings(xmlFile); // warning
                    } else {
                        // -- error
                        printError("Domain Host/Alias already defined: [" + xmlFile + " : " + privLabel.getName() + "] " + hostAlias + " ==> " + dupPL.getName());
                        this._setHasParsingErrors(xmlFile); // error
                    }
                    hasDuplicates = true;
                }
            }
    ...
    protected BasicPrivateLabel createPrivateLabel(File xmlFile, String className, String hostName)
    {
        if (StringTools.isBlank(className)) {
            ...
            try {
                ...
                printError("BasicPrivateLabel creation error ["+xmlFile+"]: " + className + " [" + t);
                this._setHasParsingErrors(xmlFile);
                return this.createPrivateLabel(xmlFile, hostName);
            }
        }
