...
//     -Added ability to filter 'geozoneID's on load/import.
//     -Added "overwrite" option for load/import (defaults to false)
// ----------------------------------------------------------------------------
    ...
    extends AccountRecord<Geozone>
{
    ...
    **/
    ...
    public static boolean isPointRadius(Geozone z)
    {
        if (z != null) {
            int gzType = z.getZoneType();
            return (gzType == GeozoneType.POINT_RADIUS.getIntValue())? true : false;
        } else {
            return false;
        }
    }

    public static boolean isPolygon(Geozone z)
    {
        if (z != null) {
            int gzType = z.getZoneType();
            return (gzType == GeozoneType.POLYGON.getIntValue())? true : false;
        } else {
            return false;
        }
    }
    ...
    **/
    ...
    private static DBField FieldInfo[] = {
        ...
        // -- Common fields
        newField_isActive(), // FLD_isActive
    ...
    *** @param iconMap  The icon pushpinID map
    *** @param dft      The default pushpin index if the pushpinID is not found or blank
    *** @return The pushpin index, or 'dft' if not found
    **/
    public int getPushpinIconIndex(OrderedMap<String,PushpinIcon> iconMap, int dft)
    {
        return EventData._getPushpinIconIndex(this.getIconName(),iconMap,dft);
    }
    ...
    **/
    public int getPushpinIconIndex(OrderedMap<String,PushpinIcon> iconMap)
    {
        return this.getPushpinIconIndex(iconMap,-1);
    }
    ...
    **/
    ...
    /**
    *** Converts the specified Geozone to a polygon, in place.
    *** Will return false if the Geozone is already a polygon.
    *** If the specified Geozone is a Bounded-Rectangle, it will be converted to a 4-vertice polygon.
    *** If the specified Geozone is a point-radius, the first point-radius will be converted to a polygon
    *** with the specified number of vertices.
    *** @param gz       The Geozone to convert (in place)
    *** @param ptCount  The preferred number of vertices
    *** @return True if the Geozone was converted to polygon, false otherwise.
    **/
    public static boolean convertToPolygon(Geozone gz, int ptCount)
    {
        ...
        if (zoneType == GeozoneType.POLYGON.getIntValue()) {
            return false;
        }

        /* current vertices */
        GeoPoint gp[] = gz.getGeoPoints();
        ...
        if (ptCount > Geozone.GetMaxVerticesCount()) {
            // -- maximum number of vertices
            ptCount = Geozone.GetMaxVerticesCount(); // maximum number of vertices
        }

        /* convert Bounded-Rectangle to Polygon */
        if (zoneType == GeozoneType.BOUNDED_RECT.getIntValue()) {
            if (gp.length >= 2) { 
                double latN =  -90.0, latS =   90.0;
                double lonW =  180.0, lonE = -180.0;
                for (int i = 0; i < gp.length; i++) {
                    double lat = gp[i].getLatitude();
                    if (lat > latN) { latN = lat; }
                    if (lat < latS) { latS = lat; }
                    double lon = gp[i].getLongitude();
                    if (lon > lonE ) { lonE  = lon; }
                    if (lon < lonW ) { lonW  = lon; }
                }
                GeoPoint newGP[] = new GeoPoint[] {
                    new GeoPoint(latN,lonW),
                    new GeoPoint(latN,lonE),
                    new GeoPoint(latS,lonE),
                    new GeoPoint(latS,lonW)
                };
                gz.setGeoPoints(newGP);
                gz.setZoneType(GeozoneType.POLYGON);
                return true;
            } else {
                return false;
            }
        ...
        /* convert Point-Radius to Polygon */
        if (zoneType == GeozoneType.POINT_RADIUS.getIntValue()) {
            // -- only the first point-radius is converted
            if (gp.length >= 1) { 
                GeoPoint centerGP = gp[0];
                int  radiusM = gz.getRadius();
                GeoPoint newGP[] = new GeoPoint[ptCount];
                for (int H = 0; H < ptCount; H++) {
                    double deg = (double)H * (360.0 / (double)ptCount);
                    newGP[H] = centerGP.getHeadingPoint(radiusM, deg);
                }
                gz.setGeoPoints(newGP);
                gz.setZoneType(GeozoneType.POLYGON);
                return true;
    ...
    **/
    ...
    /**
    *** Returns an array of active Geozones found in the specified location that are 
    *** applicable to the specified DeviceID.  
    *** @since 2.6.0-B47
    *** @param gza       The list of Geozones
    *** @param deviceID  The device id.
    ...
    **/
    ...
    /** 
    *** Returns a map of the first (highest priority) Geozone in each arrive/depart category, 
    *** in which the point resides that is also applicable for the specified DeviceID.
    *** @since 2.6.0-B47
    *** @param acctID The AccountID
    *** @param gp     The GeoPoint
    *** @param devID  The DeviceID
    ...
    **/
    ...
    /**
    *** Returns true if the subdivision-code for this geozone is defined
    **/
    public boolean hasSubdivision()
    {
        return !StringTools.isBlank(this.getSubdivision())? true : false;
    }
    ...
    **/
        ...
        String  devGrpID[])
    {
        ...
        /* limit to specific device groups? */
        if (!ListTools.isEmpty(devGrpID)) {
            dwh.append(dwh.AND_(dwh.INLIST(Geozone.FLD_groupID,devGrpID)));
        }
    ...
    {
        boolean debug = false;
        if (debug) { Print.logInfo("Debug: Searching for Geozone at " + gp); }

        /* invalid account */
        if (StringTools.isBlank(acctID)) {
            if (debug) { Print.logError("Debug: AccountID is null"); }
            return null;
        }

        /* invalid GeoPoint */
        if ((gp == null) || !gp.isValid()) {
            if (debug) { Print.logError("Debug: GeoPoint is invalid"); }
            return null;
        }
        ...
        if (debug) { Print.logInfo("Debug: Geozone Select: " + dsel); }
        ...
        if (ListTools.isEmpty(gz)) {
            if (debug) { Print.logWarn("Debug: No Geozones found in bounding box"); }
            return null;
        }
        ...
        for (int g = 0; g < gz.length; g++) {
            ...
            if (!gz[g].getIsActive()) {
                ...
                if (debug) { Print.logInfo("Debug: Geozone inactive: " + gz[g].getGeozoneID()); }
                continue;
            }
            ...
            if (debug) { Print.logInfo("Debug: Geozone does not contain point: " + gz[g].getGeozoneID()); }

        }

        /* no Geozone found */
        if (debug) { Print.logInfo("Debug: No Geozones found containing point"); }
        return null;
    ...
    /**
    *** Gets array of upload Geozone IDs
    **/
    public static String[] getClientUploadZoneIDs(String acctId)
        throws DBException
    {
        ...
        dsel.setSelectedFields(
            Geozone.FLD_accountID, 
            Geozone.FLD_geozoneID, 
            Geozone.FLD_clientID, 
            Geozone.FLD_sortID);
        ...
        /* read Geozone IDs */
        java.util.List<String> gzList = new Vector<String>();
        ...
        try {
            ...
            while (rs.next()) {
                String gzID = rs.getString(FLD_geozoneID);
                gzList.add(gzID);
            }

        } catch (SQLException sqe) {
            throw new DBException("Get Geozone IDs", sqe);
        } finally {
        ...
        /* no Geozones? */
        if (ListTools.isEmpty(gzList)) {
            return null;
        }

        /* return Geozone IDs */
        return gzList.toArray(new String[gzList.size()]);

    }

    /**
    *** Gets array of upload Geozones
    **/
    ...
    public static String[] getGeozoneIDsForAccount(String acctId)
        throws DBException
    {
        return Geozone.getGeozoneIDsForAccount(acctId, null);
    }

    /* return list of all Geozone IDs owned by the specified Account (NOT SCALABLE) */
    // -- does not return null
    public static String[] getGeozoneIDsForAccount(String acctId, String devGrpID[])
        throws DBException
    {
    ...
    public static class ZoneLoadValidator
        implements DBLoadValidator
    {
        private String      accountID        = null;
        private String      fields[]         = null;
        private int         accountID_ndx    = -1;
        private int         geozoneID_ndx    = -1;
        private int         zoneType_ndx     = -1;
        private int         count            = 0;
        private int         errors           = 0;
        private boolean     filterIDs        = false;
        private boolean     uniquify         = true;
        private boolean     forceInvalid     = false;
        private boolean     showFilteredCSV  = true;
        private boolean     convertToPolygon = false;
        private Set<String> geozones         = new HashSet<String>();
        public ZoneLoadValidator(String acctID, boolean filterIDs, boolean forceInvalid) {
            this.accountID    = (acctID != null)? acctID : "";
            this.filterIDs    = filterIDs;
            this.forceInvalid = forceInvalid;
        }
        // -- set to convert to polygon
        public void setConvertToPolygon(boolean convert) {
            this.convertToPolygon = convert;
        }
        ...
        public boolean setFields(String f[]) throws DBException { 
            ...
            // -- find accountID,geozoneID,zoneType fields
            for (int i = 0; i < this.fields.length; i++) {
                if ((this.accountID_ndx < 0) && this.fields[i].equals(FLD_accountID)) {
                    this.accountID_ndx = i;
                }
                if ((this.geozoneID_ndx < 0) && this.fields[i].equals(FLD_geozoneID)) {
                    this.geozoneID_ndx = i;
                }
                if ((this.zoneType_ndx < 0) && this.fields[i].equals(FLD_zoneType)) {
                    this.zoneType_ndx = i;
                }
            }
            // -- validate
            if (this.accountID_ndx < 0) {
                throw new DBException("Load file is missing column: " + FLD_accountID);
            } else
            if (this.geozoneID_ndx < 0) {
                throw new DBException("Load file is missing column: " + FLD_geozoneID);
            }
            // -- display header?
            if (this.showFilteredCSV && this.forceInvalid && this.filterIDs) {
                // -- is validating and filtering, display filtered CSV header
                String csv = StringTools.encodeCSV(this.fields);
                Print.sysPrintln(csv);
            }
            return true;
        }
        ...
        public boolean validateValues(String v[]) throws DBException {
            ...
            // -- valid accountID
            {
                ...
                if (this.accountID_ndx >= 0) {
                    if (this.filterIDs) {
                        a = AccountRecord.getFilteredID(v[this.accountID_ndx],false/*noNull*/,true/*lowerCase*/,true/*strict*/);
                        if (a.length() > ID_LEN) { a = a.substring(0,ID_LEN); }
                        v[this.accountID_ndx] = a;
                    } else {
                        a = v[this.accountID_ndx];
                    }
                }
                // -- validate
                if (StringTools.isBlank(a)) {
                    Print.logError("Blank AccountID found: [#" + this.count + "] " + a);
                    this.errors++;
                    return false;
                } else
                if (!a.equals(this.accountID)) {
                    Print.logError("AccountID does not match specified Account: [#" + this.count + "] " + a);
                    this.errors++;
                    return false;
                } else
                if (!AccountRecord.isValidID(a)) {
                    Print.logError("Invalid AccountID found: [#" + this.count + "] " + a);
                    this.errors++;
                    return false;
                } else
                if (a.length() > ID_LEN) {
                    Print.logError("AccountID exceeds maximum ID length: [#" + this.count + "] " + a);
                    this.errors++;
                    return false;
                }
            }
            // -- valid geozoneID
            {
                ...
                if (this.geozoneID_ndx >= 0) {
                    if (this.filterIDs) {
                        z = AccountRecord.getFilteredID(v[this.geozoneID_ndx],false/*noNull*/,true/*lowerCase*/,true/*strict*/);
                        z = StringTools.replace(z,"_-_","_");
                        z = StringTools.replace(z,"__" ,"_");
                        z = StringTools.replace(z,"--" ,"-");
                        z = StringTools.replace(z,".." ,".");
                        z = StringTools.replace(z,"@@" ,"@");
                        z = StringTools.replace(z,"."  ,"" ); // remove periods (a valid id char)
                        if (z.length() > ID_LEN) { z = z.substring(0,ID_LEN); }
                        if (this.uniquify && this.geozones.contains(z)) {
                            // -- found duplicate, uniquify ID ...
                            if (z.length() > (ID_LEN-2)) { z = z.substring(0,ID_LEN-2); } // room for 2-digits
                            for (int i = 2; i <= 99; i++) {
                                String n = String.valueOf(i);
                                while (n.length() < 2) { n = "0" + n; }
                                String zz = z + n;
                                if (!this.geozones.contains(zz)) {
                                    z = zz;
                                    break;
                                }
                            }
                        }
                        v[this.geozoneID_ndx] = z;
                    } else {
                        z = v[this.geozoneID_ndx];
                    }
                }
                // -- validate
                if (StringTools.isBlank(z)) {
                    Print.logError("Blank GeozoneID found: [#" + this.count + "] " + z);
                    this.errors++;
                    return false;
                } else
                if (!AccountRecord.isValidID(z)) {
                    Print.logError("Invalid GeozoneID found: [#" + this.count + "] " + z);
                    this.errors++;
                    return false;
                } else
                if (z.length() > 32) {
                    Print.logError("GeozoneID exceeds maximum ID length: [#" + this.count + "] " + z);
                    this.errors++;
                    return false;
                } else
                if (this.geozones.contains(z)) {
                    Print.logError("Duplicate GeozoneID: [#" + this.count + "] " + z);
                    this.errors++;
                    return false;
                }
                // -- save in cache
            ...
            if (this.forceInvalid) {
                if (this.showFilteredCSV && this.filterIDs) {
                    // -- is validating and filtering, display filtered CSV line
                    String csv = StringTools.encodeCSV(v);
                    Print.sysPrintln(csv);
                }
                return false;
            }
        ...
        public boolean validateInsert(DBRecord<?> dbr) throws DBException {
            if ((dbr != null) && this.convertToPolygon) {
                Geozone gz = (Geozone)dbr;
                Geozone.convertToPolygon(gz, 6);
            }
            return true;
        }
    ...
    private static final String ARG_FIELDS[]    = new String[] { "fields"             }; // -fields=accountID,geozoneID,...

    private static final String ARG_LOAD[]      = new String[] { "load"    , "import" }; // -load=<file>
    private static final String ARG_OVERWRITE[] = new String[] { "overwrite"          }; // -overwrite=true
    private static final String ARG_VALIDATE[]  = new String[] { "validate"           }; // -validate=<file>
    private static final String ARG_FILTERID[]  = new String[] { "filterID", "filter" }; // -filter=true
    private static final String ARG_POLYGON[]   = new String[] { "polygon"            }; // -polygon=true
    ...
    private static void usage()
    {
        ...
        Print.logInfo("  -account=<id>       Account ID which owns the specified Geozone");
        ...
        Print.logInfo("  -validate=<csvFile> Validate Geozones CSV file before loading");
        Print.logInfo("  -test=<lat>/<lon>   Test Geozone: find specified point");
        Print.logInfo("Load/Validation options:");
        Print.logInfo("  -overwrite          Overwrite existing Geozones");
        Print.logInfo("  -filterID           Adjust GeozoneID to conform to a valid ID format");
      //Print.logInfo("  -polygon            Convert point-radius geozone to 6-vertice polygon");
        System.exit(1);
    }
    ...
    public static void main(String argv[])
    {
        ...
        if (RTConfig.hasProperty(ARG_DUMP)) {
            ...
            try {
                ...
                dsel.setOrderByFields(FLD_geozoneID, FLD_sortID);
                fact.dumpTable(dumpFile, false/*append*/, dsel);
            } catch (DBException dbe) {
        ...
        if (RTConfig.hasProperty(ARG_EXPORT)) {
            opts++;
            String xportFields[] = RTConfig.getStringArray(ARG_FIELDS,null);
            ...
            try {
                ...
                dsel.setWhere(dwh.WHERE_(dwh.EQ(Geozone.FLD_accountID,acctID)));
                dsel.setOrderByFields(FLD_geozoneID, FLD_sortID);
                String exportFlds[] = !ListTools.isEmpty(xportFields)?
                   xportFields : DBFactory.getFieldNames(fact.getFieldsWithBoolean("export",true));
                fact.dumpTable(dumpFile, false/*append*/, dsel, exportFlds);
            } catch (DBException dbe) {
        ...
        if (RTConfig.hasProperty(ARG_LOAD) || RTConfig.hasProperty(ARG_VALIDATE)) {
            ...
            boolean filterIDs    = RTConfig.getBoolean(ARG_FILTERID,false); // filter/adjust geozoneID
            boolean makePolygon  = RTConfig.getBoolean(ARG_POLYGON,false);  // convert to polygon
            ...
            try {
                if (fact.tableExists()) {
                    String mode = validateOnly? "Validate" : "Load";
                    Print.sysPrintln(mode + " '" + TABLE_NAME() + "' file " + loadFile);
                    ZoneLoadValidator zlv = new ZoneLoadValidator(acctID,filterIDs,validateOnly);
                    zlv.setConvertToPolygon(makePolygon);
                    fact.loadTable(loadFile,zlv,!validateOnly/*insertRecord*/,overwrite,false/*noDropWarning*/);
                    if (zlv.hasErrors()) {
                        Print.sysPrintln(mode+" errors encountered: '" + TABLE_NAME() + "' file " + loadFile);
                    } else {
                        Print.sysPrintln(mode+" successful: '" + TABLE_NAME() + "' file " + loadFile);
                    }
                } else {
                ...
                Print.logException("Exception loading/validating table: " + TABLE_NAME(), dbe);
            }
            System.exit(0);
        }
        ...
        if (RTConfig.hasProperty(ARG_TEST)) {
            // -- bin/admin.pl Geozone -account=demo -test=39.1234/-142.1234
            opts++;
