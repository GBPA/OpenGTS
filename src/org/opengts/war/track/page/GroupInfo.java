    ...
    implements Constants
{
    ...
    public  static final String PARM_DEV_MEMBER_            = "g_dev_";
    ...
    private String _queueDeviceGroupProperties(DeviceGroup selGroup, User userAuth, HttpServletRequest request, I18N i18n)
    {
        ...
        for (int d = 0; d < devList.size(); d++) {
            try {
                ...
                for (Iterator<PendingPacket.SetPropertyPacket> i = propList.iterator(); i.hasNext();) {
                    PendingPacket.SetPropertyPacket spp = i.next();
            ...
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
                  ...
                  //String menuURL    = EncodeMakeURL(reqState,RequestProperties.TRACK_BASE_URI(),PAGE_MENU_TOP);
                    ...
                    /* Devices in group */
                    String _sortByStr = privLabel.getStringProperty(PrivateLabel.PROP_GroupInfo_deviceListSortBy,"desc");
                    IDDescription.SortBy sortBy = IDDescription.GetSortBy(_sortByStr);
                    {
                        // -- read-only view
                    ...
                    if (_editGroup) {
                        ...
                        out.write("<input type='button' class='btn btn-default' name='"+PARM_BUTTON_BACK+"' value='"+i18n.getString("GroupInfo.back","Back")+"' onclick=\"javascript:openURL('"+editURL+"','_self');\">\n"); // target='_top'
                    }
                    out.write("</form>\n");
