...
//     -Modified to dynamically present commands from dcserver_SERVERID.xml
// ----------------------------------------------------------------------------
    ...
    implements DeviceCmdHandler
{
    ...
    private Map<String,DCServerConfig.Command> _getCommandMap(
        DCServerConfig dcs, BasicPrivateLabel privLabel, User user, Device device,
        String type)
    {
        ...
        /* return command map */
        Map<String,DCServerConfig.Command> cmdMap = dcs.getCommandMap(privLabel, user, device, type);
    ...
    private Map<String,String> getCommandDescriptionMap(
        BasicPrivateLabel privLabel, User user, Device device,
        String type)
    {
        DCServerConfig dcs = DeviceCmd_SMS.getServerConfig(null, privLabel);
        if (dcs != null) {
            return dcs.getCommandDescriptionMap(privLabel, user, device, type);
        } else {
            Print.logInfo("DCServer not found: " + DEVICE_CODE);
            return null;
        }
    ...
    **/
    private RTProperties sendCommand(Device device, DCServerConfig.Command command, String cmdArgs[])
    {
        ...
        /* send SMS message */
        DCServerFactory.ResultCode result = DCServerFactory.SendSMSCommand(device, smsCmd);
