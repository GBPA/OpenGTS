...
//     -Check for "admin" userID if User does not exist [2.6.4-B40]
// ----------------------------------------------------------------------------
    ...
    implements Constants
{
  ...
  //private static final boolean SEND_ACCOUNT_LIST          = true;
        ...
        String contactEmail)
    {
        ...
        SmtpProperties smtpProps = privLabel.getSmtpProperties(); // SmtpProperties ??
        ...
        /* send */
        String from = smtpProps.getFromEmailType("password");
    ...
    private static String getContactEMailAddress(String accountID, String userID)
    {
        ...
        if (!StringTools.isBlank(userID)) {
            try {
                ...
                if (user != null) {
                    // -- User found, return user contact email address
                } else
                if (User.isAdminUser(userID)) { // [2.6.4-B40]
                    // -- implied "admin" User, return Account contact email address
                } else {
                    // -- User not found, and not "admin", return null
        ...
        String contactEmail)
    {
        ...
        if (!StringTools.isBlank(userID)) {
            try {
                ...
                if (User.isAdminUser(userID)) { // [2.6.4-B40]
                    // -- continue below with implied "admin" user from Account
                } else {
        ...
        long passwdQueryTime = (user != null)? user.getPasswdQueryTime() : account.getPasswdQueryTime();
        ...
        /* get password */
        String decodedPass = (user != null)? user.getDecodedPassword(privLabel) : account.getDecodedPassword(privLabel);
        if (decodedPass == null) {
            // -- password cannot be decoded, instead reset
            try {
                if (user != null) {
                    decodedPass = user.resetPassword(privLabel); // not yet saved
                    user.updatePasswordFields(); // now save
                } else {
                    decodedPass = account.resetPassword(privLabel); // not yet saved
                    account.updatePasswordFields(); // now save
                }
            } catch (DBException dbe) {
