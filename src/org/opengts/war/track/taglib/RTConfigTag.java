    ...
    implements Constants, StringTools.KeyValueMap
{
    ...
    private static final String KEY_START_FTL           = "@{";
    private static final String KEY_START_JSP           = StringTools.KEY_START; // ${
    ...
    /**
    *** Gets the replacement key-start prefix
    *** @return The replacement key-start prefix
    **/
    public String getReplacementKeyStart()
    {
        return this.isFTL()? KEY_START_FTL : KEY_START_JSP;
    }

    /**
    *** Gets the replacement key-end prefix
    *** @return The replacement key-end prefix
    **/
    public String getReplacementKeyEnd()
    {
        return KEY_END;
    }
    ...
    **/
    ...
    /**
    *** Returns true if we are running in a Freemarker template environment
    **/
    public boolean isFTL()
    {
        ...
        return (rp != null)? rp.isWebPageTemplateFTL() : false;
    }
    ...
    **/
    ...
    /**
    *** "<code>StringTools.KeyValueMap</code>" interface.
    *** Callback from "<code>StringTools.replaceKeys</code>"
    **/
    ...
    **/
    ...
    private void setSavedBodyContent(String body)
    {
        ...
        if (!StringTools.isBlank(this.bodyContent) && Character.isLetter(this.bodyContent.charAt(0))) {
            String keyStart = this.getReplacementKeyStart();
            String keyEnd   = this.getReplacementKeyEnd();
            int e = this.bodyContent.indexOf(keyEnd);
            if (e >= 0) {
                int s = this.bodyContent.indexOf(keyStart);
                if ((s <= -1) || (s > e)) {
                    this.bodyContent = keyStart + this.bodyContent;
        ...
        try {
            if (this.isMatch()) {
                String body = StringTools.replaceKeys(this.getSavedBodyContent(), 
                    ...
                    keyStart, keyEnd, ARG_DELIM, DFT_DELIM);
