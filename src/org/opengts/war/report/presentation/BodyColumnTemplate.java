...
public class BodyColumnTemplate
{
    ...
    {
        ColumnValue cv = (valObj instanceof ColumnValue)? (ColumnValue)valObj : null; // HTML
        ...
        if ((cv != null) && cv.hasLinkURL()) {
            ...
            if (linkURL.startsWith("javascript:")) {
                if (cv.isLinkButton()) {
                    out.print("<button class='buttonLink' onclick=\"" + linkURL + "\">");
                    out.print(cellValue);
                    out.print("</button>");
                } else {
    ...
    {
        ColumnValue cv = (valObj instanceof ColumnValue)? (ColumnValue)valObj : null; // XML
