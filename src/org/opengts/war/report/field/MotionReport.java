...
//     -Added "fd.setDevice(..)" to detail rows to allow for displaying device description. [2.6.1-B27]
//  2016/06/10  Martin D. Flynn
//     -Added initial support for "FieldLayout.DATA_SPEED_MAXIMUM" [2.6.2-B74]
// ----------------------------------------------------------------------------
    ...
    implements DBRecordHandler<EventData>
{
    ...
    **/
    ...
    private double                      maxAchievedSpeedK       = 0.0;      // maximum achieved speed
    ...
    private double                      totalMaxSpeedK          = 0.0;      // auto-reset per device
    ...
    **/
    public DBDataIterator getBodyDataIterator()
    {
        ...
        double grandTotalMaxSpeedK = 0.0;
        // -- Work Hours
        ...
        for (Iterator<String> i = devList.iterator(); i.hasNext();) {
            ...
            // -- reset maximum achieved speed
            this.maxAchievedSpeedK   = 0.0;
            // -- reset motion
            ...
            try {
                ...
                if (this.lastStopTime > 0) {
                    ...
                    double fuelIdle  = -1.0;
                    double maxSpeedK = (this.maxAchievedSpeedK > 0.0)? this.maxAchievedSpeedK : 0.0; // final trip stop
                    this._addRecord( // getBodyDataIterator
                        ...
                        stopElaps, idleElaps, fuelIdle, maxSpeedK);
                } else
                ...
                fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED      , this.totalDriveSec);
                ...
                fd.setDouble(FieldLayout.DATA_SPEED_MAXIMUM     , this.totalMaxSpeedK);
                // -- Work Hours
                fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED_WH   , this.tworkDriveSec);
                ...
                // -- After Hours
                fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED_AH   , (this.totalDriveSec - this.tworkDriveSec));
                ...
                grandTotalMaxSpeedK  = Math.max(grandTotalMaxSpeedK,this.totalMaxSpeedK);
                // -- Work Hours
        ...
        if (this.isFleetReport) {
            ...
            fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED       , grandTotalDriveSec);
          ...
          //fd.setValue( FieldLayout.DATA_FUEL_ECONOMY_TYPE  , avgEconType);
            ...
            fd.setDouble(FieldLayout.DATA_SPEED_MAXIMUM      , grandTotalMaxSpeedK);
            // -- Work Hours
            fd.setDouble(FieldLayout.DATA_ODOMETER_DELTA_WH  , gworkTotalOdomKM);
            fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED_WH    , gworkTotalDriveSec);
            ...
            fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED_AH    , gafterTotalDriveSec);
    ...
    *** @param maxSpeedK     The maximum speed achieved (<='0.0' if unavailable)
    **/
    private void _addRecord(
        ...
        long   stopElapse, long idleElapse , double fuelIdle    , double maxSpeedK
        )
    {

        /* standard fields */
        FieldData fd = new MotionFieldData();
        fd.setDevice(device); // [2.6.1-B27]
        ...
        fd.setLong(    FieldLayout.DATA_TRIP_ELAPSED     , driveTime);
        ...
        fd.setDouble(  FieldLayout.DATA_SPEED_MAXIMUM    , maxSpeedK);
        ...
        // -- Work Hours
        fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED_WH   , whDriveTime);
        ...
        // -- After Hours
        fd.setLong(  FieldLayout.DATA_TRIP_ELAPSED_AH   , (driveTime - whDriveTime));
        ...
        if (maxSpeedK   > 0.0) { this.totalMaxSpeedK   = Math.max(this.totalMaxSpeedK,maxSpeedK); }
        ...
        throws DBException
    {
        ...
        if (this.tripStartType == TRIP_ON_IGNITION) {
            ...
            if (this.isIgnitionOn(statusCode)) {
                ...
                if ((this.lastIgnitionEvent == null) || !this.isIgnitionOn) {
                    // -- ignition was previously off, OR I did not receive a prior ignition event
                    ignitionChange              = true;
                    this.isIgnitionOn           = true;
                  //this.lastIgnOnEvent         = evRcd;
                    this.lastIgnitionEvent      = evRcd;
            ...
            if (this.isIgnitionOff(statusCode)) {
                ...
                if ((this.lastIgnitionEvent == null) || this.isIgnitionOn) {
                    // -- ignition was previously on, OR I did not receive a prior ignition event
                    ignitionChange              = true;
                    this.isIgnitionOn           = false;
                  //this.lastIgnOffEvent        = evRcd;
                    this.lastIgnitionEvent      = evRcd;
        ...
        // ignitionChange           - true if ignition changed state (from On==>Off or Off==>On)
        ...
        /* accrue maximum achieved speed */
        if (speedKPH > this.maxAchievedSpeedK) {
            this.maxAchievedSpeedK = speedKPH;
        }

        /* add detail records for trip stop */
        ...
        if (isMotionStart) {
            ...
            if (this.lastStateChange == STATE_START) {
                ...
                long     idleElaps = 0L;
                double   fuelIdle  = -1.0;
                double   maxSpeedK = (this.maxAchievedSpeedK > 0.0)? this.maxAchievedSpeedK : 0.0; // trip stop
        ...
        if (isMotionStop) {
            ...
            /* check for abnormal previous stop condition */
            if (this.lastStateChange == STATE_STOP) {
                ...
                if ((this.lastStopTime > 0) && (this.lastIgnitionEvent != null) && (this.lastIgnitionEvent.getTimestamp() > this.lastStopTime)) {
                    ...
                    long     idleElaps = (this.idleAccumulator > 0L)? this.idleAccumulator : -1L;
                    double   fuelIdle  = -1.0;
                    double   maxSpeedK = (this.maxAchievedSpeedK > 0.0)? this.maxAchievedSpeedK : 0.0; // duplicate trip stop
                    this._addRecord( // handleDBRecord
                        _deviceDetailData,
                        ev.getAccountID(), ev.getDeviceID(), device,
    ...
    **/
        ...
        implements EventDataProvider
    {
        ...
        public int getPushpinIconIndex(String iconSelector, OrderedMap<String,PushpinIcon> iconMap, 
            ...
            return PushpinIcon.ICON_PUSHPIN_RED;
        }
