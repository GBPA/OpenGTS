...
//    gprmc.logName              - Name used in logging output
//    gprmc.uniquePrefix         - Prefix used on uniqueID when lookup up Device [defaults to 'gprmc.logName']
//    gprmc.defaultAccountID     - Default account id
//    gprmc.minimumSpeedKPH      - Minimum acceptable speed
//    gprmc.dateFormat           - Date format for 'date' parameter (NONE|EPOCH|YMD|DMY|MDY)
//    gprmc.response.ok          - Response on successful data
//    gprmc.response.error       - Response on error data
//    gprmc.parm.mobile          - Mobile-ID parameter key
//    gprmc.parm.account         - Account-ID parameter key
//    gprmc.parm.device          - Device-ID parameter key
//    gprmc.parm.auth            - Auth/Password parameter key (not used)
//    gprmc.parm.status          - StatusCode parameter key 
//    gprmc.parm.gprmc           - $GPRMC parameter key 
//    gprmc.parm.date            - Date parameter key (ignored if 'gprmc' is used) 
//    gprmc.parm.time            - Time parameter key (ignored if 'gprmc' is used) 
//    gprmc.parm.latitude        - Latitude parameter key (ignored if 'gprmc' is used)
//    gprmc.parm.longitude       - Longitude parameter key (ignored if 'gprmc' is used) 
//    gprmc.parm.speed"          - Speed(kph) parameter key (ignored if 'gprmc' is used) 
//    gprmc.parm.heading         - Heading(degrees) parameter key (ignored if 'gprmc' is used)
//    gprmc.parm.altitude        - Altitude(meters) parameter key
//    gprmc.parm.sats            - Number of satellites key
//    gprmc.parm.hdop            - Horizontal Dilution of Precision
//    gprmc.parm.hacc            - Horizontal Accuracy
//    gprmc.parm.vacc            - Vertical Accuracy
//    gprmc.parm.odometer        - Odometer(kilometers) parameter key
//    gprmc.parm.address         - Reverse-Geocode parameter key
//    gprmc.parm.driver          - DriverID parameter key 
//    gprmc.parm.message         - Message parameter key 
//    gprmc.parm.battlevel       - Battery Level
//    gprmc.parm.battvolts       - Battery Volts
//    gprmc.parm.batttemp        - Battery Temperature
//    gprmc.parm.temperature     - Temperature #0
//    gprmc.parm.mcc             - Mobile Country Code 
//    gprmc.parm.mnc             - Mobile Network Code
//    gprmc.parm.lac             - Location Area Code
//    gprmc.parm.cid             - Cell Tower ID 
//    gprmc.parm.tav             - Timing Advance 
//    gprmc.parm.rat             - Radio Access Technology
//    gprmc.parm.rxlev           - Reception Level
//    gprmc.parm.arfcn           - Absolutie Radio-Freq Channel Number 
//    gprmc.parm.email           - Contact Email (not used)
//
...
//    gprmc.parm.hdop=hdop
//    gprmc.parm.hacc=hacc,accuracy,acc
//    gprmc.parm.vacc=vacc
...
//    gprmc.parm.tav=tav
//    gprmc.parm.rat=rat
//    gprmc.parm.rxlev=rxlev,rxlvl,rxl
//    gprmc.parm.arfcn=arfcn,arf
...
//      acct=myaccount&
//      dev=mydevice&
//      gprmc=$GPRMC,065954,A,3244.2749,N,14209.9369,W,21.6,0.0,211202,11.8,E,S*1D
...
//     -Added PARM_SPEED_KNOTS [2.6.4-B13]
//  2017/07/21  Martin D. Flynn
//     -Added support for "dataSource" and "signalStrength" [2.6.5-B34]
// ----------------------------------------------------------------------------
    ...
    extends CommonServlet
{
    ...
    /* DCS version */
    public static final String VERSION                      = "2.2.11"; // 2.6.5-B13
    ...
    /* special fields */
    private static String FIELD_PREFIX_                     = "$";
    private static String FIELD_VIN                         = "$VIN";
    private static String FIELD_TIRE_STATE                  = "$TireState";
    ...
    private static String PARM_DATE[]                       = { "date" , "datetime","dt","utcdate"   };  // date (DateFormat)
    private static String PARM_TIME[]                       = { "time" , "ts","tm","utctime"         };  // time (HHMMSS)
    ...
    private static String PARM_SPEED_KPH[]                  = { "speed", "kph","speed_kph"           };  // speed (km/h)
    private static String PARM_SPEED_MPH[]                  = { "mph"  , "speedmph","speed_mph"      };  // speed (mph)
    private static String PARM_SPEED_KNOTS[]                = { "knots", "speedknots","speed_knots"  };  // speed (knots)
    private static String PARM_HEADING[]                    = { "head" , "heading","direction","dir","course","course_deg" };  // heading (degrees)
    ...
    public static final String  CONFIG_PARM_SPEED_KNOTS     = DEVICE_CODE + ".parm.speedknots";     // knots
    ...
    private static EventDataAnalog FieldAnalog[] = null; 
    ...
    static {
        ...
        PARM_SPEED_KNOTS     = RTConfig.getStringArray(CONFIG_PARM_SPEED_KNOTS     , PARM_SPEED_KNOTS    );
        ...
        Data.printHeaderLine("SpeedKNOTS parameter"   , PARM_SPEED_KNOTS      , CONFIG_PARM_SPEED_KNOTS);
    ...
    **/
    ...
    private static Map<String,Object> PutField(Map<String,Object> flds, String fn, Object val)
    {
        if (flds == null) {
    ...
    private static Map<String,Object> PutField(Map<String,Object> flds, String fn, String val)
    {
        return PutField(flds, fn, (Object)val);
    }

    private static Map<String,Object> PutField(Map<String,Object> flds, String fn, double val)
    {
        return PutField(flds, fn, new Double(val));
    }

    private static Map<String,Object> PutField(Map<String,Object> flds, String fn, long val)
    {
        return PutField(flds, fn, new Long(val));
    }

    private static Map<String,Object> PutField(Map<String,Object> flds, String fn, int val)
    {
        return PutField(flds, fn, new Integer(val));
    }

    private static Map<String,Object> PutField(Map<String,Object> flds, String fn, boolean val)
    {
        return PutField(flds, fn, new Boolean(val));
    }
        ...
        throws ServletException, IOException
    {
        ...
        if (isPost) {
            ...
            for (java.util.Enumeration<String> ae = request.getParameterNames(); ae.hasMoreElements();) {
                if (postSB.length() > 0) { postSB.append("&"); }
                String ak = ae.nextElement();
        ...
        String _speedKNOTS = AttributeTools.getRequestString(request, PARM_SPEED_KNOTS   , null);
        ...
        double speedKNOTS  = StringTools.parseDouble(_speedKNOTS, 0.0);         // knots
        ...
        /* parse extra values */
        Map<String,Object> extraFlds = null;
        ...
        if ((speedKPH <= 0.0) && (speedKNOTS > 0.0)) {
            speedKPH = speedKNOTS * GeoPoint.KILOMETERS_PER_NAUTICAL_MILE;
        } else
        ...
        if (!StringTools.isBlank(gprmcStr)) {
            ...
            fixtime    = this._parseFixtime(dateStr, timeStr, DateFormat);
        ...
        if (!ListTools.isEmpty(extraFlds)) {
            for (String key : extraFlds.keySet()) {
                Object val = extraFlds.get(key);
        ...
        if (SIMEVENT_GEOZONES && isValidGPS) {
            ...
            if (zone != null) {
                for (Device.GeozoneTransition z : zone) {
                    ...
                    //zoneEv.setEmailRecipient(emailAddr);
                    if (!ListTools.isEmpty(extraFlds)) {
                        for (String fld : extraFlds.keySet()) {
                            if (fld.equalsIgnoreCase(FIELD_VIN)) {
                                // -- handle VIN
                                String vin = (String)extraFlds.get(fld);
                                device.setVehicleID(vin);
                                device.addOtherChangedFieldNames(Device.FLD_vehicleID);
                            } else
                            if (fld.equalsIgnoreCase(FIELD_TIRE_STATE)) {
                                // -- handle TireState
                                @SuppressWarnings("unchecked")
                                Collection<TireState> tsList = (Collection<TireState>)extraFlds.get(fld);
                                zoneEv.setTireState(tsList);
                            } else
                            if (!fld.startsWith(FIELD_PREFIX_)) {
                                // -- EventData field
                                Object val = extraFlds.get(fld);
                                zoneEv.setFieldValue(fld, val);
                            } else {
                                // -- ignore field
                            }
                        }
    ...
    private long _parseFixtime(String dateStr, String timeStr, int dateFmt)
    {
        // Examples:
        ...
        if (((dateFmt == DATE_FORMAT_YMDhms) || (dateFmt == DATE_FORMAT_YMD)) &&
