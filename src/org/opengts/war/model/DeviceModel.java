...
import org.opengts.war.track.page.DeviceCmdHandler;
    ...
    implements Constants
{
    ...
    // -- forms
    public  static final String FORM_DEVICE_SELECT          = "DeviceModelSelect";
    public  static final String FORM_DEVICE_EDIT            = "DeviceModelEdit";
    public  static final String FORM_DEVICE_NEW             = "DeviceModelNew";
  ...
  //public  static final String PARM_WORKORDER_ID           = "d_workid";
    ...
    private static void _addDeviceCommandHandler(DeviceCmdHandler dch)
    {
        ...
        if (!_DCMap.containsKey(servID)) {
            ...
            DeviceModel._DCMap.put(servID, dch);
        } else {
            DeviceCmdHandler dupDCH = DeviceModel._DCMap.get(servID);
    ...
    private static void _initDeviceCommandHandlers()
    {
        ...
        synchronized (DeviceModel._DCMap) { // [2.6.1-B21]
            if (!didInitDeviceCommandHandlers) {
                ...
                */
                ...
                if (DeviceCmd_SMS_class != null) {
                    ...
                    for (DCServerConfig dcs : dcServerList) { // for (String _dchn : optDevCmdHandlerClasses) 
                        ...
                        // DeviceModel.DeviceCmdAlternate.calamp=1,2,3
                        int p = _dchn.indexOf(":");
                        ...
                        if (dchClass != null) {
                            ...
                            Print.logWarn("DeviceCommand Page class not found: " + dchClassName);
                            continue;
                                ...
                                if (!StringTools.isBlank(arg)) {
                                    if (dchClass != null) {
                                        try {
                                            ...
                                            DeviceModel._addDeviceCommandHandler(argDCH);
    ...
    private DeviceCmdHandler getDeviceCommandHandler(String dcid)
    {
        ...
        DeviceCmdHandler dch = DeviceModel._getDCMap().get(dcid); // may still be null
        if (dch == null) {
    ...
    private static long GetEpochTimeDayStart(DateTime.DateStringFormat dsf, String dat, TimeZone tz)
    {
        ...
        return DeviceModel.GetEpochTime(ymd, tz, DateTime.DefaultParsedTime.DayStart);
    }

    private static long GetEpochTimeDayEnd(DateTime.DateStringFormat dsf, String dat, TimeZone tz)
    {
        String ymd = (dsf != null)? dsf.convertToYMD(dat) : dat;
        return DeviceModel.GetEpochTime(ymd, tz, DateTime.DefaultParsedTime.DayEnd);
    }
    ...
    private static long GetDayNumberTS(String ymd, TimeZone tmz, boolean dayStart)
    {
        if (StringTools.isBlank(ymd)) {
            return 0L;
        } else {
    ...
    public String getMenuDescription(RequestProperties reqState, String parentMenuName)
    {
        ...
        I18N i18n = privLabel.getI18N(DeviceModel.class);
    ...
    public String getMenuHelp(RequestProperties reqState, String parentMenuName)
    {
        ...
        I18N i18n = privLabel.getI18N(DeviceModel.class);
        String devTitles[] = reqState.getDeviceTitles();
        return super._getMenuHelp(reqState,i18n.getString("DeviceModel.editMenuHelp","View and Edit {0} information", devTitles));
    }
    ...
    public String getNavigationDescription(RequestProperties reqState)
    {
        ...
        I18N i18n = privLabel.getI18N(DeviceModel.class);
        String devTitles[] = reqState.getDeviceTitles();
        return super._getNavigationDescription(reqState,i18n.getString("DeviceModel.navDesc","{0}", devTitles));
    }
    ...
    public String getNavigationTab(RequestProperties reqState)
    {
        ...
        I18N i18n = privLabel.getI18N(DeviceModel.class);
        String devTitles[] = reqState.getDeviceTitles();
        return i18n.getString("DeviceModel.navTab","{0} Admin", devTitles);
    }
    ...
    */
      ...
      //String  worderID    = AttributeTools.getRequestString(request, PARM_WORKORDER_ID           , "");
        try {
                    ...
                    try {
                        ...
                        if (dev == null) {
                            ...
                            if (devAcctID.equals(reqState.getCurrentAccountID())) {
                                ...
                                msg = i18n.getString("DeviceModel.uniqueIdAlreadyAssignedToDevice","UniqueID is already assigned to {0}: {1}", devTitles[0], devDevID); // UserErrMsg
                                selDev.setError(msg);
                            } else {
                                ...
                                msg = i18n.getString("DeviceModel.uniqueIdAlreadyAssigned","UniqueID is already assigned to another Account"); // UserErrMsg
                                selDev.setError(msg);
                            }
                        }
                    } catch (DBException dbe) {
                        msg = i18n.getString("DeviceModel.errorReadingUniqueID","Error while looking for other matching UniqueIDs"); // UserErrMsg
                        selDev.setError(msg);
                    }
                }
            }
            ...
            if (devExpOK && editExpire) {
                long expireTS = GetDayNumberTS(devExpire, acctTimeZone, true);
            ...
            if (Device.supportsMapShare()) {
                ...
                if (shareMapOK) {
                    ...
                    long shareStartTS = GetDayNumberTS(shareStart, acctTimeZone, true);
                    long shareEndTS   = GetDayNumberTS(shareEnd  , acctTimeZone, false);
                ...
                if (uniqSimPhone) {
                    ...
                    if (devListCnt == 1) {
                        ...
                        if (!acctDevID.equalsIgnoreCase(devAcctID + sep + devDevID)) {
                            ...
                            msg = i18n.getString("DeviceModel.simPhoneNotUnique","SIM phone number is not unique: " + simPhone);
                            selDev.setError(msg);
                            simPhone = "";
                        }
                    } else {
            ...
            if (dataKeyReq && StringTools.isBlank(dataKey)) {
                msg = i18n.getString("DeviceModel.dataKeyRequired","Non-blank 'Data Key' entry required");
                selDev.setError(msg);
            } else
              ...
              //String  ruleWrap    = AttributeTools.getRequestString(  request, PARM_DEV_RULE_WRAP   ,  null);
                ...
                if (notifyEmail != null) {
                    ...
                    if (EMail.validateAddresses(notifyEmail,true)) {
                        ...
                        msg = i18n.getString("DeviceModel.enterEMail","Please enter a valid notification email/sms address");
                        selDev.setError(msg);
                    }
                }
                // -- notification selector
                if (ruleSel != null) {
                    if (DeviceModel.ShowNotifySelector() && !Device.CheckSelectorSyntax(ruleSel)) {
                        Print.logInfo("Notification selector has a syntax error: " + ruleSel);
                        msg = i18n.getString("DeviceModel.ruleError","Notification rule contains a syntax error");
                        selDev.setError(msg);
                    } else {
            ...
            */
            ...
            if (selDev.hasChanged()) {
                selDev.save();
                msg = i18n.getString("DeviceModel.updatedDevice","{0} information updated", devTitles);
            } else
            if (groupsChg) {
                String grpTitles[] = reqState.getDeviceGroupTitles();
                msg = i18n.getString("DeviceModel.updatedDeviceGroups","{0} membership updated", grpTitles);
            } else {
                // -- nothing changed
            ...
            msg = i18n.getString("DeviceModel.errorUpdate","Internal error updating {0}", devTitles);
            selDev.setError(msg);
        }
        return msg;
    }
        ...
        throws IOException
    {
        ...
        if (!allowSmsCmd && Print.isDebugLoggingLevel()) {
            ...
            if (currAcct == null) {
                Print.logDebug("  SMS Commands: Current Account is null");
            }
            if (!currAcct.getSmsEnabled()) {
                Print.logDebug("  SMS Commands: SMS disabled for Current Account");
            }
            if (!allowView) {
                Print.logDebug("  SMS Commands: Page view disabled");
            }
            if (!privLabel.hasWriteAccess(currUser,this.getAclName(_ACL_SMS))) {
                Print.logDebug("  SMS Commands: Does not have write access to ACL " + this.getAclName(_ACL_SMS));
            }
            if (SMSCommandHandler == null) {
                Print.logDebug("  SMS Commands: SMSCommandHandler is null");
            }
            if ((SMSCommandHandler != null) && !SMSCommandHandler.hasSmsCommands(reqState)) { // DeviceCmd_SMS
                Print.logDebug("  SMS Commands: SMSCommandHandler has no SMS commands");
            }
        }
      ...
      //boolean refreshList  = deviceCmd.equals(COMMAND_INFO_REFRESH);
        ...
        if (newDevice) {
            ...
            if (overDevLimit > 0) {
                ...
                if (StringTools.isBlank(newDeviceID)) {
                    m = i18n.getString("DeviceModel.enterNewDevice","Please enter a new {0} ID.", devTitles); // UserErrMsg
                    error = true;
                    newDevice = false;
                } else
                if (!WebPageAdaptor.isValidID(reqState,/*PrivateLabel.PROP_DeviceInfo_validateNewIDs,*/newDeviceID)) {
                    m = i18n.getString("DeviceModel.invalidIDChar","ID contains invalid characters"); // UserErrMsg
                    error = true;
                    newDevice = false;
                }
            }
        ...
        if (updateDevice) {
            ...
            if (!SubmitMatch(submitChange,i18n.getString("DeviceModel.change","Change"))) {
                updateDevice = false;
            } else
            if (selDev == null) {
                // should not occur
                m = i18n.getString("DeviceModel.unableToUpdate","Unable to update Device, ID not found"); // UserErrMsg
                error = true;
                updateDevice = false;
            }
        } else
        if (sendCommand) {
            ...
            if (!SubmitMatch(submitQueue,i18n.getString("DeviceModel.queue","Queue"))) {
                sendCommand = false; // button not pressed
            } else
            if (selDev == null) {
                m = i18n.getString("DeviceModel.pleaseSelectDevice","Please select a {0}", devTitles); // UserErrMsg
                error = true;
        ...
        if (selectDevice) {
            if (SubmitMatch(submitDelete,i18n.getString("DeviceModel.delete","Delete"))) {
                if (!allowDelete) {
                ...
                if (!actualDev) {
                    ...
                    m = i18n.getString("DeviceModel.unableToDelete","Unable to delete selected {0}", devTitles); // UserErrMsg
                    error = true;
                    deleteDevice = false; // not selected
                } else {
                    deleteDevice = true;
            ...
            if (SubmitMatch(submitEdit,i18n.getString("DeviceModel.edit","Edit"))) {
                if (!allowEdit) {
            ...
            if (SubmitMatch(submitView,i18n.getString("DeviceModel.view","View"))) {
                if (!allowView) {
            ...
            if (SubmitMatch(submitProps,i18n.getString("DeviceModel.properties","Commands"))) {
                if (!allowCommand) {
                ...
                if (selDev == null) {
                    ...
                    if (dcHandler == null) {
                        if (StringTools.isBlank(dcsName)) {
                            ...
                            m = i18n.getString("DeviceModel.deviceCommandsNotDefined","Unknown {0} type (blank)", devTitles); // UserErrMsg
                        } else {
                            Print.logWarn("DeviceCmdHandler not found: " + dcsName);
                            m = i18n.getString("DeviceModel.deviceCommandsNotFound","{0} not supported", devTitles); // UserErrMsg
                        }
                        error = true;
                    ...
                    if (!dcHandler.deviceSupportsCommands(selDev)) {
                        ...
                        m = i18n.getString("DeviceModel.deviceCommandsNotSupported","{0} Commands not supported", devTitles); // UserErrMsg
                        error = true;
            ...
            if (SubmitMatch(submitSms,i18n.getString("DeviceModel.sms","SMS"))) {
                if (!allowSmsCmd) {
                ...
                if (selDev == null) {
                    ...
                    if (dcHandler == null) {
                        ...
                        m = i18n.getString("DeviceModel.deviceSmsNotSupported","{0} SMS not supported", devTitles); // UserErrMsg
                        error = true;
        ...
        if (deleteDevice) {
            ...
            try {
                ...
                m = i18n.getString("DeviceModel.errorDelete","Internal error deleting {0}", devTitles); // UserErrMsg
                error = true;
            }
            uiList = true;
        }
        ...
        if (newDevice) {
            ...
            for (int d = 0; d < devList.size(); d++) {
                if (newDeviceID.equalsIgnoreCase(devList.get(d))) {
                    m = i18n.getString("DeviceModel.alreadyExists","This {0} already exists", devTitles); // UserErrMsg
                    error = true;
            ...
            if (createDeviceOK) {
                try {
                    ...
                    // -- message
                  //m = i18n.getString("DeviceModel.createdUser","New {0} has been created", devTitles); // UserErrMsg
                    m = i18n.getString("DeviceModel.createdDevice","New {0} has been created", devTitles); // UserErrMsg
                    ...
                    m = i18n.getString("DeviceModel.errorCreate","Internal error creating {0}", devTitles); // UserErrMsg
                    error = true;
                }
            }
            uiList = true;
        ...
        if (_uiList) {
            ...
            model = new DataModel(this,m,error) {
                public void initModel() {
                    String selectURL  = DeviceModel.this.encodePageURL(reqState);//,RequestProperties.TRACK_BASE_URI());
                    String refreshURL = DeviceModel.this.encodePageURL(reqState);
                  //String editURL    = DeviceModel.this.encodePageURL(reqState);//,RequestProperties.TRACK_BASE_URI());
                    String newURL     = DeviceModel.this.encodePageURL(reqState);//,RequestProperties.TRACK_BASE_URI());

                    /* digital input state */
                    Vector<StateMaskBit> inpStateList = null;
                    for (int i = 0; i < 16; i++) {
                        // -- DeviceModel.showInputState.01=Panic,Off,On
                    ...
                    for (int i = 0; i < 16; i++) {
                        // -- DeviceModel.showOutputState.01=Panic,Off,On
                    ...
                    if (_allowEdit) {
                        ...
                        this.setFrameTitle(i18n.getString("DeviceModel.viewEditDevice","View/Edit {0} Information", devTitles));
                    } else {
                        this.setPageContext(DataModel.CONTEXT_VIEW); // EDIT|VIEW|LIST|COMMANDS|etc
                        this.setFrameTitle(i18n.getString("DeviceModel.viewDevice","View {0} Information", devTitles));
                    }

                    // -- list selection header
                    SelectionListModel slm = this.getSelectionListModel();
                    slm.setForm("DeviceModel_List", selectURL);
                    slm.setFormTitle(i18n.getString("DeviceModel.selectDevice","Select a {0}",devTitles));
                    slm.addFormButton(SelectionListModel.PROP_btnRefresh,null,i18n.getString("DeviceModel.refresh","Refresh"),"javascript:openURL('"+refreshURL+"','_self');");
                  //slm.addInputHidden(PARM_COMMAND, COMMAND_INFO_SEL_DEVICE);
                    // -- Header
                    SelectionListModel.Columns slmh = slm.getHeader();
                    slmh.addColumn(i18n.getString("DeviceModel.select","Select"));
                    slmh.addColumn(i18n.getString("DeviceModel.deviceID","{0} ID",devTitles));
                    if (_viewUniqID) {
                        slmh.addColumn(i18n.getString("DeviceModel.uniqueID","Unique ID"));
                    }
                    slmh.addColumn(i18n.getString("DeviceModel.decription","Description",devTitles));
                    slmh.addColumn(i18n.getString("DeviceModel.devEquipType","Equipment\nType"));
                    if (_viewSIM) {
                        slmh.addColumn(i18n.getString("DeviceModel.simPhoneNumber","SIM Phone#"));
                    }
                    if (_viewServID) {
                        slmh.addColumn(i18n.getString("DeviceModel.devServerID","Server ID"));
                    }
                    slmh.addColumn(i18n.getString("DeviceModel.ignitionState","Ignition\nState"));
                    if (!ListTools.isEmpty(inpStateList)) {
                        for (StateMaskBit smb : inpStateList) {
                            slmh.addColumn(smb.getTitle());
                        }
                    }
                    ...
                    if (showAcks) {
                        slmh.addColumn(i18n.getString("DeviceModel.ackExpected","Expecting\nACK"));
                    }
                    if (_viewActive) {
                        slmh.addColumn(i18n.getString("DeviceModel.active","Active"));
                    }
                    // -- Rows
                    ...
                    for (int d = 0; d < _deviceList.size(); d++) {
                        try {
                            ...
                            if (dev != null) {
                                ...
                                boolean _checked  = _selDevID.equals(dev.getDeviceID());
                                String checked    = _checked? "checked" : "";
                                SelectionListModel.Columns slmr = slm.newRow();
                                slmr.addColumn(deviceID).setSelect(PARM_DEVICE,deviceID,_checked).setSortKey(d);
                                slmr.addColumn(deviceID).setLabelFor(deviceID);
                                if (_viewUniqID) {
                                    slmr.addColumn(uniqueID);
                                }
                                slmr.addColumn(deviceDesc);
                                slmr.addColumn(equipType);
                                if (_viewSIM) {
                                    slmr.addColumn(simPhone);
                                }
                                if (_viewServID) {
                                    slmr.addColumn(ignDesc);
                                }
                                slmr.addColumn(devCode);
                                slmr.addColumn(ignDesc).setStyle("color:"+ignColor);
                                if (!ListTools.isEmpty(inpStateList)) {
                                    for (StateMaskBit smb : inpStateList) {
                                        String state_val = smb.getStateDescription(dev.getLastInputState(smb.getBitIndex()));
                                        slmr.addColumn(state_val);
                                    }
                                }
                                if (!ListTools.isEmpty(outStateList)) {
                                    for (StateMaskBit smb : outStateList) {
                                ...
                                if (showAcks) {
                                    slmr.addColumn(pendingACK).setStyle("color:red");
                                }
                                if (_viewActive) {
                                    slmr.addColumn(active);
                                }
                                ...
                                if (DeviceModel.this.getDeviceCommandHandler(serverID) != null) {
                    ...
                    if (_allowSmsCmd) {  // SMS commands
                        slm.addFormSubmit(FormModel.PROP_btnSms, PARM_SUBMIT_SMS, i18n.getString("DeviceModel.sms","SMS"));
                    }
                    if (_allowDelete) {
                        slm.addFormSubmit(FormModel.PROP_btnDelete, PARM_SUBMIT_DEL, i18n.getString("DeviceModel.delete","Delete")).setOnClick(Onclick_ConfirmDelete(locale));
                    }

                    /* new device */
                    if (_allowNew && (_overDevLimit <= 0)) {
                        String createText = i18n.getString("DeviceModel.createNewDevice","Create a new device");
                        if (allowNewDevMode == DEVMODE_SYSADMIN) {
                            createText += " " + i18n.getString("DeviceModel.sysadminOnly","(System Admin only)");
                        } else
                        if (allowNewDevMode == DEVMODE_MANAGER) {
                            createText += " " + i18n.getString("DeviceModel.managerOnly","(AccountManager only)");
                        }
                      //this.createNewRecord(createText, newURL);
                      /*
                ...
                public void write(PrintWriter out) throws IOException {
                    ...
                    if (ppAccounts[0].equals("*") || ListTools.contains(ppAccounts,currAcctID)) {
                        ...
                        for (String ppKey : ppKeys) {
                            ...
                            // -  "DeviceModel.custom.DEVICEKEY" ==> "DEVICEKEY"
                    ...
                    if (StringTools.isBlank(lastConnectTime)) { lastConnectTime = i18n.getString("DeviceModel.neverConnected","never"); }
                    ...
                    if (StringTools.isBlank(deviceCode)) {
                        autoFill = i18n.getString("DeviceModel.serverIdAutoFill","(automatically entered by the DCS)");
                    }
                    if (_viewServID) {
                        out.println(FormRow_TextField(PARM_SERVER_ID        , edServID_  , i18n.getString("DeviceModel.serverID","Server ID")+":"               , deviceCode, 18, 20, autoFill));
                    }
                    if (_viewCodeVer) {
                        out.println(FormRow_TextField(PARM_CODE_VERS        , edCodeVer_ , i18n.getString("DeviceModel.firmwareVers","Firmware Version")+":"    , firmVers, 28, 28));
                    }
                    if (_viewUniqID) {
                        out.println(FormRow_TextField(PARM_DEV_UNIQ         , _editUniqID, i18n.getString("DeviceModel.uniqueID","Unique ID")+":"               , (_selDev!=null)?_selDev.getUniqueID():""       , 30, 30));
                    }
                    if (_viewActive) {
                        out.println(FormRow_ComboBox (PARM_DEV_ACTIVE       , _editActive, i18n.getString("DeviceModel.active","Active")+":"                    , devActive, ComboMap.getYesNoMap(locale), ""    , -1));
                    }
                    if (devExpOK && _viewExpire) {
                        String expTitle = i18n.getString("DeviceModel.deviceExpire","{0} Expiration",devTitles) + ":";
                        DateTime.DateStringFormat dsf = privLabel.getDateStringFormat();
                        ...
                        if (showDateCal) {
                            ...
                            String extraYMD = i18n.getString("DeviceModel.dateYMD","(yyyy/mm/dd)");
                    ...
                    out.println(FormRow_TextField(PARM_DEV_DESC         , _uiEdit    , i18n.getString("DeviceModel.deviceDesc","{0} Description",devTitles) +":",(_selDev!=null)?_selDev.getDescription():"" , 40, 64));
                    out.println(FormRow_TextField(PARM_DEV_NAME         , _uiEdit    , i18n.getString("DeviceModel.displayName","Short Name") +":"          , (_selDev!=null)?_selDev.getDisplayName():""    , 16, 64));
                    out.println(FormRow_TextField(PARM_VEHICLE_ID       , _uiEdit    , i18n.getString("DeviceModel.vehicleID","Vehicle ID") +":"            , (_selDev!=null)?_selDev.getVehicleID():""      , 24, 24));
                    out.println(FormRow_TextField(PARM_VEHICLE_MAKE     , _uiEdit    , i18n.getString("DeviceModel.vehicleMake","Vehicle Make") +":"        , (_selDev!=null)?_selDev.getVehicleMake():""    , 40, 40));
                    out.println(FormRow_TextField(PARM_VEHICLE_MODEL    , _uiEdit    , i18n.getString("DeviceModel.vehicleModel","Vehicle Model") +":"      , (_selDev!=null)?_selDev.getVehicleModel():""   , 40, 40));
                    // -- license plate/expire
                    out.println(FormRow_TextField(PARM_LICENSE_PLATE    , _uiEdit    , i18n.getString("DeviceModel.licensePlate","License Plate") +":"      , (_selDev!=null)?_selDev.getLicensePlate():""   , 16, 24));
                    if (licExpOK) {
                        String licTitle = i18n.getString("DeviceModel.licenseExpire","License Expiration") + ":";
                        DateTime.DateStringFormat dsf = privLabel.getDateStringFormat();
                    ...
                    // -- Equipment type/status
                    out.println(FormRow_TextField(PARM_DEV_EQUIP_TYPE   , _uiEdit    , i18n.getString("DeviceModel.equipmentType","Equipment Type") +":"    , (_selDev!=null)?_selDev.getEquipmentType():""  , 30, 40));
                    if (_viewEqStat) {
                        String eqTitle = i18n.getString("DeviceModel.equipmentStatus","Equipment Status") + ":";
                        ...
                        if (!ListTools.isEmpty(eqStatMap)) {
                            ...
                            if (!esList.containsKeyIgnoreCase(devEqSt)) {
                                String desc = !StringTools.isBlank(devEqSt)? devEqSt : i18n.getString("DeviceModel.blank","(blank)");
                    ...
                    if (_viewIMEI) {
                        out.println(FormRow_TextField(PARM_DEV_IMEI         , _editIMEI  , i18n.getString("DeviceModel.imeiNumber","IMEI/ESN Number") +":"      , (_selDev!=null)?_selDev.getImeiNumber():""     , 18, 24));
                    }
                    if (_viewSERIAL) {
                        out.println(FormRow_TextField(PARM_DEV_SERIAL_NO    , _editSERIAL, i18n.getString("DeviceModel.serialNumber","Serial Number") +":"      , (_selDev!=null)?_selDev.getSerialNumber():""   , 20, 24));
                    }
                    if (dataKeyOK && _viewDATKEY) {
                        out.println(FormRow_TextField(PARM_DATA_KEY         , _editDATKEY, i18n.getString("DeviceModel.dataKey","Data Key") +":"                , (_selDev!=null)?_selDev.getDataKey():""        , 60, 200));
                    }
                    if (_viewSIM) {
                        out.println(FormRow_TextField(PARM_DEV_SIMPHONE     , _editSIM   , i18n.getString("DeviceModel.simPhoneNumber","SIM Phone#") +":"       , (_selDev!=null)?_selDev.getSimPhoneNumber():"" , 14, 18));
                    }
                    if (_viewSMSEm) {
                        out.println(FormRow_TextField(PARM_SMS_EMAIL        , _editSMSEm , i18n.getString("DeviceModel.smsEmail","SMS Email Address") +":"      , (_selDev!=null)?_selDev.getSmsEmail():""       , 60, 60));
                    }
                    if (ppidOK) {
                        String ppDesc = i18n.getString("DeviceModel.mapPushpinID","{0} Pushpin ID",grpTitles)+":";
                    ...
                    if (dcolorOK) {
                        String dcDesc = i18n.getString("DeviceModel.mapRouteColor","Map Route Color")+":";
                        String dcolor = (_selDev != null)? _selDev.getDisplayColor() : "";
                        double P = 0.30;
                        ComboMap dcCombo = new ComboMap();
                        dcCombo.add(""                                        ,i18n.getString("DeviceModel.color.default","Default"));
                        dcCombo.add(ColorTools.BLACK.toString(true)           ,i18n.getString("DeviceModel.color.black"  ,"Black"  ));
                        dcCombo.add(ColorTools.BROWN.toString(true)           ,i18n.getString("DeviceModel.color.brown"  ,"Brown"  ));
                        dcCombo.add(ColorTools.RED.toString(true)             ,i18n.getString("DeviceModel.color.red"    ,"Red"    ));
                        dcCombo.add(ColorTools.ORANGE.darker(P).toString(true),i18n.getString("DeviceModel.color.orange" ,"Orange" ));
                      //dcCombo.add(ColorTools.YELLOW.darker(P).toString(true),i18n.getString("DeviceModel.color.yellow" ,"Yellow" ));
                        dcCombo.add(ColorTools.GREEN.darker(P).toString(true) ,i18n.getString("DeviceModel.color.green"  ,"Green"  ));
                        dcCombo.add(ColorTools.BLUE.toString(true)            ,i18n.getString("DeviceModel.color.blue"   ,"Blue"   ));
                        dcCombo.add(ColorTools.PURPLE.toString(true)          ,i18n.getString("DeviceModel.color.purple" ,"Purple" ));
                        dcCombo.add(ColorTools.DARK_GRAY.toString(true)       ,i18n.getString("DeviceModel.color.gray"   ,"Gray"   ));
                      //dcCombo.add(ColorTools.WHITE.toString(true)           ,i18n.getString("DeviceModel.color.white"  ,"White"  ));
                        dcCombo.add(ColorTools.CYAN.darker(P).toString(true)  ,i18n.getString("DeviceModel.color.cyan"   ,"Cyan"   ));
                        dcCombo.add(ColorTools.PINK.toString(true)            ,i18n.getString("DeviceModel.color.pink"   ,"Pink"   ));
                        dcCombo.add("none"                                    ,i18n.getString("DeviceModel.color.none"   ,"None"   ));
                    ...
                    if (ignOK) {
                        ...
                        out.println(FormRow_ComboBox( PARM_IGNITION_INDEX, _uiEdit   , i18n.getString("DeviceModel.ignitionIndex","Ignition Input") +":" , ignSel, ignList, "", -1, i18n.getString("DeviceModel.ignitionIndexDesc","(ignition input line, if applicable)")));
                    }
                    // -- Speed Limit
                    if (speedLimOK) {
                        ...
                        out.println(FormRow_TextField(PARM_DEV_SPEED_LIMIT, _uiEdit  , i18n.getString("DeviceModel.speedLimit","Maximum Speed") +":"            , speedLimStr                                    , 10, 10, speedUnitsStr));
                    }
                    // -- Driver ID
                    out.println(FormRow_TextField(PARM_DRIVER_ID, _uiEdit, i18n.getString("DeviceModel.driverID","Driver ID")+":", (_selDev!=null)?_selDev.getDriverID():""       , 24, 30));
                    // -- User ID
                    if (showAssgnUsr && Device.supportsAssignedUserID()) {
                      ...
                      //out.println(FormRow_TextField(PARM_USER_ID, _uiEdit, i18n.getString("DeviceModel.userID","Assigned User ID")+":", userSel , 24, 24));
                        out.println(FormRow_ComboBox(PARM_USER_ID, _uiEdit, i18n.getString("DeviceModel.userID","Assigned User ID")+":", userSel, userCombo, "", -1));
                    }
                    // ------------------------------------
                    //-- Fuel Section
                    // -- fuel capacity
                    ...
                    if (showFuelEcon && _viewFuelEco) {
                        ...
                        out.println(FormRow_TextField(PARM_DEV_FUEL_ECON   , _editFuelEco, i18n.getString("DeviceModel.fuelEconomy","Fuel Economy") +":"        , fuelEconStr, 10, 10, econUnitsStr));
                    }
                    // -- fuel cost
                    if (showFuelCost && _viewFuelCst) {
                        ...
                        out.println(FormRow_TextField(PARM_DEV_FUEL_COST   , _editFuelCst, i18n.getString("DeviceModel.fuelCost","Fuel Cost") +":"              , fuelCostStr, 10, 10, costUnitsStr));
                    }
                    ...
                    if (showFuelCap && _viewFuelCap) {
                        ...
                        out.println(FormRow_TextField(PARM_DEV_FUEL_CAP1  , _editFuelCap, i18n.getString("DeviceModel.fuelCapacity","Fuel Tank #1 Capacity") +":", fuelCap1Str, 10, 10, volmUnitsStr));
                        // -- Tank #2
                        ...
                        out.println(FormRow_TextField(PARM_DEV_FUEL_CAP2  , _editFuelCap, i18n.getString("DeviceModel.fuelCapacity2","Fuel Tank #2 Capacity") +":", fuelCap2Str, 10, 10, volmUnitsStr));
                    }
                    // -- fuel tank profile
                    if (showFuelProf && _viewFuelPrf) {
                        ...
                        switch (fuelProfType) {
                            ...
                            case 1: { // "text"
                                ...
                                // -- #1
                                out.println(FormRow_SubSeparator());
                                out.println(FormRow_TextField(PARM_DEV_FUEL_PROFILE1, _editFuelPrf, i18n.getString("DeviceModel.fuelProfile1","Fuel Tank #1 Profile")+":", profSel1, 80, 250));
                                // -- #2
                                out.println(FormRow_SubSeparator());
                                out.println(FormRow_TextField(PARM_DEV_FUEL_PROFILE2, _editFuelPrf, i18n.getString("DeviceModel.fuelProfile2","Fuel Tank #2 Profile")+":", profSel2, 80, 250));
                                } break;
                            case 2: { // "select"
                                // -- view as ComboBox
                                ...
                                // -- #1
                                out.println(FormRow_SubSeparator());
                                out.println(FormRow_ComboBox( PARM_DEV_FUEL_PROFILE1, _editFuelPrf, i18n.getString("DeviceModel.fuelProfile1","Fuel Tank #1 Profile")+":", profSel1, flpCombo, "", -1));
                                // -- #1
                                out.println(FormRow_SubSeparator());
                                out.println(FormRow_ComboBox( PARM_DEV_FUEL_PROFILE2, _editFuelPrf, i18n.getString("DeviceModel.fuelProfile2","Fuel Tank #2 Profile")+":", profSel2, flpCombo, "", -1));
                                } break;
                            case 3: { // "scale"
                                // -- view as "Linear"/"Cylider" pull-down and scale/offset text
                                OrderedMap<String,String> profMap = new OrderedMap<String,String>();
                                profMap.put(FuelLevelProfile.FLP_NONE_NAME    , i18n.getString("DeviceModel.None"    ,"None"    ));
                                profMap.put(FuelLevelProfile.FLP_LINEAR_NAME  , i18n.getString("DeviceModel.Linear"  ,"Linear"  ));
                                profMap.put(FuelLevelProfile.FLP_CYLINDER_NAME, i18n.getString("DeviceModel.Cylinder","Cylinder"));
                                ComboMap flpCombo = new ComboMap(profMap);
                                // -- #1
                                {
                                ...
                                out.println(FormRow_TextField(PARM_DEV_FUEL_PROFILE1_OFFS , _editFuelPrf, i18n.getString("DeviceModel.fuelProfile1Offset","Fuel Tank #1 Profile Offset")+":", offset1, 5, 5));
                                }
                                // -- #2
                                {
                                ...
                                out.println(FormRow_TextField(PARM_DEV_FUEL_PROFILE2_OFFS , _editFuelPrf, i18n.getString("DeviceModel.fuelProfile2Offset","Fuel Tank #2 Profile Offset")+":", offset2, 5, 5));
                                }
                                } break;
                        }
                    }
                    ...
                    if (lastOdomOK && Device.supportsLastOdometer()) {
                        ...
                        String ofsText  = i18n.getString("DeviceModel.offset","Offset");
                        String odom_ofs = distUnitsStr;
                        odom_ofs += " [" + ofsText + " " + ofsStr + "]";
                        out.println(FormRow_TextField(PARM_REPORT_ODOM, _uiEdit, i18n.getString("DeviceModel.reportOdometer","Reported Odometer") +":" , odomStr, 10, 11, odom_ofs));
                    }
                    if (lastEngHrsOK && Device.supportsLastEngineHours()) {
                      ...
                      //String lastHrT  = i18n.getString("DeviceModel.lastEngineHours","Last Engine Hours");
                        String ofsStr   = StringTools.format(offsetHr,"0.00");
                        String ofsText  = i18n.getString("DeviceModel.offset","Offset");
                        String hour_ofs = i18n.getString("DeviceModel.hours","Hours");
                        hour_ofs += " [" + ofsText + " " + ofsStr + "]";
                        out.println(FormRow_TextField(PARM_REPORT_HOURS, _uiEdit, i18n.getString("DeviceModel.reportEngineHours","Reported Engine Hours") +":" , hourStr, 10, 11, hour_ofs));
                    }
                    if (Device.supportsPeriodicMaintenance()) {
                        double offsetKM = (_selDev != null)? _selDev.getOdometerOffsetKM() : 0.0;
                        double offsetHR = (_selDev != null)? _selDev.getEngineHoursOffset() : 0.0;
                        ...
                        if (maintNotesOK) { // Domain.Properties.deviceInfo.showMaintenanceNotes=false
                            ...
                            out.println(FormRow_TextArea(PARM_MAINT_NOTES, _uiEdit, i18n.getString("DeviceModel.maintNotes" ,"Maintenance Notes")+":", noteText, 3, 70));
                        }
                        ...
                        if (maintOdomOK) { // Domain.Properties.deviceInfo.showMaintenanceOdometer=false
                            for (int ndx = 0; ndx < Device.getPeriodicMaintOdometerCount(); ndx++) {
                                ...
                                out.println(FormRow_TextField(PARM_MAINT_LASTKM_+ndx, false, i18n.getString("DeviceModel.mainLast","Last Maintenance #{0}",ndxStr) +":" , lastMaintSt, 10, 11, distUnitsStr));
                                double intrvKM = (_selDev != null)? _selDev.getMaintIntervalKM(ndx) : 0.0;
                                double intrv   = distUnits.convertFromKM(intrvKM);
                                out.print("<tr>");
                                out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap>"+i18n.getString("DeviceModel.maintInterval","Maintenance Interval #{0}",ndxStr)+":</td>");
                                ...
                                if (_uiEdit) {
                                    out.print(" &nbsp;&nbsp;(" + i18n.getString("DeviceModel.maintReset","Check to Reset Service") + " ");
                        ...
                        if (maintHoursOK) { // Domain.Properties.deviceInfo.showMaintenanceHours=false
                            for (int ndx = 0; ndx < Device.getPeriodicMaintEngHoursCount(); ndx++) {
                                ...
                                out.println(FormRow_TextField(PARM_MAINT_LASTHR_+ndx, false, i18n.getString("DeviceModel.maintEngHours","Last Eng Hours Maint",ndxStr) +":" , lastMaintSt, 10, 11));
                                double intrvHR = (_selDev != null)? _selDev.getMaintIntervalHR(ndx) : 0.0;
                                double intrv   = intrvHR;
                                out.print("<tr>");
                                out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap>"+i18n.getString("DeviceModel.maintIntervalHR","Eng Hours Maint Interval",ndxStr)+":</td>");
                                out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_DATA+"'>");
                                out.print(Form_TextField(PARM_MAINT_INTERVHR_+ndx, _uiEdit, String.valueOf((long)intrv), 10, 11));
                                out.print("&nbsp;");
                                if (_uiEdit) {
                                    out.print(" &nbsp;&nbsp;(" + i18n.getString("DeviceModel.maintResetHR","Check to Reset Service") + " ");
                        ...
                        if (reminderOK) { // Domain.Properties.deviceInfo.showReminderMessage=false
                            ...
                            out.println(FormRow_SubSeparator());
                            out.println(FormRow_TextField(PARM_REMIND_INTERVAL, _uiEdit, i18n.getString("DeviceModel.reminderInterval","Reminder Interval") +":", remIntr, 30, 60));
                            out.println(FormRow_TextArea( PARM_REMIND_MESSAGE , _uiEdit, i18n.getString("DeviceModel.reminderMessage" ,"Reminder Message")+":", remText, 3, 70));
                        }
                        ...
                        if (servTimeOK) {
                            ...
                            String lastTitle   = i18n.getString("DeviceModel.lastServiceTime","Last Service Time")+":";
                            String nextTitle   = i18n.getString("DeviceModel.nextServiceTime","Next Service Time")+":";
                            out.println(FormRow_SubSeparator());
                            if (showDateCal) {
                    ...
                    if (workHoursOK && Device.supportsHoursOfOperation()) {
                        ...
                        out.println(FormRow_SubSeparator());
                        out.println(FormRow_TextField(PARM_HOURS_OF_OPERATION, _uiEdit, i18n.getString("DeviceModel.hoursOfOperation","Hours of Operation")+":", hooStr, 100,140));
                    }
                    if (faultCodesOK && Device.supportsFaultCodes()) {
                        ...
                        out.println(FormRow_Separator());
                        out.print("<tr>");
                        out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap>"+i18n.getString("DeviceModel.faultCodes","Fault Codes")+":</td>");
                        out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_DATA+"'>");
                        out.print(Form_TextField(PARM_FAULT_CODES, false, fc, 60, 60));
                        out.print("&nbsp;");
                        if (_uiEdit) {
                            out.print(" &nbsp;&nbsp;(" + i18n.getString("DeviceModel.faultReset","Check to Reset") + " ");
                    ...
                    if (ntfyOK) {
                        ...
                        out.println(FormRow_Separator());
                        out.println(FormRow_ComboBox (PARM_DEV_RULE_ALLOW, ntfyEdit  , i18n.getString("DeviceModel.notifyAllow","Notify Enable")+":"     , allowNotfy, ComboMap.getYesNoMap(locale), "", -1));
                        out.println(FormRow_TextField(PARM_DEV_RULE_EMAIL, ntfyEdit  , i18n.getString("DeviceModel.notifyEMail","Notify Email")+":"      , (_selDev!=null)?_selDev.getNotifyEmail():"", 95, neFldLen));
                        out.print("<tr>");
                        out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap>"+i18n.getString("DeviceModel.lastAlertTime","Last Alert Time")+":</td>");
                        out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_DATA+"'>");
                        long lastNotifyTime = (_selDev != null)? _selDev.getLastNotifyTime() : 0L;
                        if (lastNotifyTime <= 0L) {
                            String na = i18n.getString("DeviceModel.noAlert","n/a");
                            ...
                            if (ntfyEdit) {
                                out.print(" &nbsp;&nbsp;(" + i18n.getString("DeviceModel.lastAlertReset","Check to Reset Alert") + " ");
                        ...
                        if (!DeviceModel.ShowNotifySelector()) {
                        ...
                        if (SHOW_NOTIFY_SELECTOR           || // always show?
                            ...
                            out.println(FormRow_TextField(PARM_DEV_RULE_SEL  , ntfyEdit  , i18n.getString("DeviceModel.ruleSelector","Rule Selector")+":"     , ntfySel  , 80, 200));
                          //out.println(FormRow_TextField(PARM_DEV_RULE_DESC , ntfyEdit  , i18n.getString("DeviceModel.notifyDesc"  ,"Notify Description")+":", ntfyDesc , 75,  90));
                            out.println(FormRow_TextField(PARM_DEV_RULE_SUBJ , ntfyEdit  , i18n.getString("DeviceModel.notifySubj"  ,"Notify Subject")+":"    , ntfySubj , 75,  90));
                            out.println(FormRow_TextArea( PARM_DEV_RULE_TEXT , ntfyEdit  , i18n.getString("DeviceModel.notifyText"  ,"Notify Message")+":"    , ntfyText ,  5,  70));
                            ...
                            //    out.println(FormRow_ComboBox(PARM_DEV_RULE_WRAP, editWrap, i18n.getString("DeviceModel.notifyWrap" ,"Notify Use Wrapper")+":", wrapEmail, ComboMap.getYesNoMap(locale), "", -1, i18n.getString("DeviceModel.seeEventNotificationEMail","(See 'EventNotificationEMail' tag in 'private.xml')")));
                            //} else {
                    ...
                    if (Device.hasENRE() && Device.supportsActiveCorridor()) {
                        ...
                        String gcTitle  = i18n.getString("DeviceModel.activeCorridor","Active Corridor") + ":";
                        String gcList[] = Device.getCorridorIDsForAccount(reqState.getCurrentAccountID());
                        out.println(FormRow_Separator());
                        if (gcList != null) {
                            ComboMap gcCombo = new ComboMap(gcList);
                            gcCombo.insert("", i18n.getString("DeviceModel.corridorNone","(none)"));
                    ...
                    if (acctBCEnabled && Device.supportsBorderCrossing()) {
                        ...
                        out.println(FormRow_Separator());
                        out.println(FormRow_ComboBox (PARM_BORDER_CROSS_ENAB, _uiEdit, i18n.getString("DeviceModel.borderCrossingEnabled","Border Crossing Enable")+":", bcOpt, ComboMap.getYesNoMap(locale), "", -1));
                        out.println(FormRow_TextField(PARM_BORDER_CROSS_TIME, false  , i18n.getString("DeviceModel.borderCrossingTime"   ,"Border Crossing Time"  )+":", bcTimeS, 30, 30));
                    }
                    // -- WorkOrder info
                    /*
                    if (_showWorkOrderID(privLabel)) {
                        String actvWO = (_selDev != null)? _selDev.getWorkOrderID() : "";
                        String woTitle = i18n.getString("DeviceModel.workOrderID","Work Order ID") + ":";
                        out.println(FormRow_Separator());
                    ...
                    */
                    ...
                    if (Device.supportsLinkURL()) {
                        out.println(FormRow_Separator());
                        out.println(FormRow_TextField(PARM_LINK_URL         , _uiEdit, i18n.getString("DeviceModel.linkURL"        ,"Link URL")         +":" , (_selDev!=null)?String.valueOf(_selDev.getLinkURL()):""        , 60, 70));
                        out.println(FormRow_TextField(PARM_LINK_DESC        , _uiEdit, i18n.getString("DeviceModel.linkDescription","Link Description") +":" , (_selDev!=null)?String.valueOf(_selDev.getLinkDescription()):"", 10, 24));
                    }
                    // -- Fixed location
                    if (fixLocOK) {
                        out.println(FormRow_Separator());
                        out.println(FormRow_TextField(PARM_FIXED_LAT        , _uiEdit, i18n.getString("DeviceModel.fixedLatitude" ,"Fixed Latitude")     +":"  , (_selDev!=null)?String.valueOf(_selDev.getFixedLatitude()) :"0.0",  9, 10));
                        out.println(FormRow_TextField(PARM_FIXED_LON        , _uiEdit, i18n.getString("DeviceModel.fixedLongitude","Fixed Longitude")    +":"  , (_selDev!=null)?String.valueOf(_selDev.getFixedLongitude()):"0.0", 10, 11));
                        out.println(FormRow_TextField(PARM_FIXED_ADDRESS    , _uiEdit, i18n.getString("DeviceModel.fixedAddress"  ,"Fixed Address")      +":"  , (_selDev!=null)?_selDev.getFixedAddress():""                     , 70, 80));
                        out.println(FormRow_TextField(PARM_FIXED_PHONE      , _uiEdit, i18n.getString("DeviceModel.fixedPhone"    ,"Fixed Contact Phone")+":"  , (_selDev!=null)?_selDev.getFixedContactPhone():""                , 30, 40));
                    }
                    // -- Shared Map start/end times
                    if (shareMapOK) {
                        out.println(FormRow_Separator());
                        String extraYMD   = i18n.getString("DeviceModel.dateYMD", "(yyyy/mm/dd)");
                        String startTitle = i18n.getString("DeviceModel.shareMapStartDay","ShareMap Start Day") + ":";
                        String endTitle   = i18n.getString("DeviceModel.shareMapEndDay"  ,"ShareMap End Day") + ":";
                        DateTime.DateStringFormat dsf = privLabel.getDateStringFormat();
                    ...
                    if (subscribeOK) {
                        out.println(FormRow_Separator());
                        out.println(FormRow_TextField(PARM_SUBSCRIBE_ID    , _uiEdit, i18n.getString("DeviceModel.subscriberID"    ,"Subscriber ID")+":"     , _selDev.getSubscriberID()    , 24, 24));
                        out.println(FormRow_TextField(PARM_SUBSCRIBE_NAME  , _uiEdit, i18n.getString("DeviceModel.subscriberName"  ,"Subscriber Name")+":"   , _selDev.getSubscriberName()  , 40, 40));
                        out.println(FormRow_TextField(PARM_SUBSCRIBE_AVATAR, _uiEdit, i18n.getString("DeviceModel.subscriberAvatar","Subscriber Avatar")+":" , _selDev.getSubscriberAvatar(), 80, 80));
                    }
                    // -- Last connect time
                    if (SHOW_LAST_CONNECT) {
                        out.println(FormRow_Separator());
                        out.println(FormRow_TextField(PARM_DEV_LAST_CONNECT , false  , i18n.getString("DeviceModel.lastConnect","Last Connect")+":" , lastConnectTime, 30, 30, i18n.getString("DeviceModel.serverTime","(Server time)"))); // read-only
                        out.println(FormRow_TextField(PARM_DEV_LAST_EVENT   , false  , i18n.getString("DeviceModel.lastEvent"  ,"Last Event"  )+":" , lastEventTime  , 30, 30, i18n.getString("DeviceModel.deviceTime","(Device time)"))); // read-only
                    }
                    // -- Notes
                    if (notesOK) {
                        String noteText = (_selDev!=null)? StringTools.decodeNewline(_selDev.getNotes()) : "";
                        out.println(FormRow_Separator());
                        out.println(FormRow_TextArea(PARM_DEV_NOTES, _uiEdit, i18n.getString("DeviceModel.notes" ,"General Notes")+":", noteText, 5, 70));
                    }
                    ...
                    if (showDCSPropID || showDCSCfgStr || showTcpSessID) {
                        ...
                        if (showDCSPropID) {
                            ...
                            String desc = i18n.getString("DeviceModel.dcsPropertiesID" ,"DCS Properties ID");
                            if (gnList != null) {
                        ...
                        if (showDCSCfgStr) {
                            String desc  = i18n.getString("DeviceModel.dcsConfigString","DCS Configuration String");
                        ...
                        if (showTcpSessID) {
                            String desc  = i18n.getString("DeviceModel.tcpSessionID","Static TCP Session ID");
                    ...
                    if ((_selDev != null) && Account.IsELogEnabled(_selDev.getAccount())) {
                        ...
                        out.println(FormRow_ComboBox(PARM_DEV_ENABLE_ELOG, _uiEdit, i18n.getString("DeviceModel.enableDriverELogs","Enable Driver ELogs")+":", elogOpt, elogMap, "", -1, elogDesc));
                        out.println(FormRow_Separator());
                    } else {
                    ...
                    if (showPrefGrp) {
                        ...
                        if (!includeAll) {
                            grpMap.insert("", i18n.getString("DeviceModel.noGroup" ,"No {0}",grpTitles));
                        }
                        String grpSel = (_selDev != null)? _selDev.getGroupID() : "";
                        out.println(FormRow_ComboBox(PARM_GROUP_ID, _uiEdit, i18n.getString("DeviceModel.groupID","Preferred {0}",grpTitles)+":", grpSel, grpMap, "", -1));
                        out.println(FormRow_SubSeparator());
                    }
