    ...
    extends BasicPrivateLabel
{
    ...
    public static class JSPEntry
    {
        ...
        public JSPEntry(String name, I18N.Text desc, String file) {
            ...
            this.desc    = desc;
            this.file    = StringTools.trim(file);
            this.cssDir  = null;
        }
    ...
    // -- favicon link
    private String                          faviconLink                 = null;
    ...
    // -- OptionalEventFields
    private volatile boolean                optionalEventFields_init    = false;
    private Object                          optionalEventFields_lock    = new Object(); // lazy lock
    private volatile OptionalEventFields    optionalEventFields         = null;
    ...
    **/
    ...
    /**
    *** Returns the first (active) MapProvider 
    *** @return The first (active) MapProverer
    **/
    ...
    **/
    public int getPushpinIconIndex(String mapProviderID, String pushpinID, int dftIndex)
    {
        ...
        if (mp != null) {
            ...
            return EventData._getPushpinIconIndex(pushpinID, iconMap, dftIndex); 
        } else {
    ...
    **/
    ...
    {
        if (!this.optionalEventFields_init) {
            // -- initialize
            synchronized (this.optionalEventFields_lock) {
                if (!this.optionalEventFields_init) {
                    // -- get MapProvider
                    MapProvider mp = this.getMapProvider();
                    RTProperties mpRTP = (mp != null)? mp.getProperties() : null;
                    // -- DeviceMap fields
                    String devFlds[];
                    for (;;) { // single-pass-loop
                        String fldsKey[] = OptionalEventFields.PROP_OptionalEventFields_DeviceMap;
                        // -- check MapProvider properties
                        if (mpRTP != null) {
                            String fldStr = mpRTP.getString(fldsKey,null);
                            if (!StringTools.isBlank(fldStr)) {
                                devFlds = StringTools.parseArray(fldStr,',');
                                break;
                            }
                        }
                        // -- check private label properties
                        if (this.hasProperty(fldsKey)) {
                            String fldStr = this.getStringProperty(fldsKey,null);
                            if (!StringTools.isBlank(fldStr)) {
                                devFlds = StringTools.parseArray(fldStr,',');
                                break;
                            }
                        }
                        // -- not found, exit single-pass-loop
                        devFlds = null;
                        break; // exit SPL
                    }
                    // -- FleetMap fields
                    String fltFlds[] = null;
                    for (;;) { // single-pass-loop
                        String fldsKey[] = OptionalEventFields.PROP_OptionalEventFields_FleetMap;
                        // -- check MapProvider properties
                        if (mpRTP != null) {
                            String fldStr = mpRTP.getString(fldsKey,null);
                            if (!StringTools.isBlank(fldStr)) {
                                fltFlds = StringTools.parseArray(fldStr,',');
                                break;
                            }
                        }
                        // -- check private label properties
                    ...
                    // -- create
                    if (!ListTools.isEmpty(devFlds) || !ListTools.isEmpty(fltFlds)) {
                        this.optionalEventFields = OptionalEventFields.createOptionalEventFieldsHandler(devFlds,fltFlds);
                         //Print.logInfo("Created OptionalEventFields=" + this.optionalEventFields);
                    }
                    // -- did init
                    this.optionalEventFields_init = true;
                }
            } // synchronized
        }
        return this.optionalEventFields;
    }
    ...
    **/
    protected String _getDefaultWebPageJSP(RequestProperties reqState)
    {
    ...
    **/
    public String getWebPageJSP(String jsp, RequestProperties reqState)
    {
        if (StringTools.isBlank(jsp)) {
            // -- return default
            return this._getDefaultWebPageJSP(reqState); // may be null
        } else
        if (jsp.startsWith("/")) {
            // -- absolute path
            return jsp;
            ...
            if (!StringTools.isBlank(jf)) {
                //Print.logInfo("Found JSP File: " + jf);
                return jf;
            } else {
                // -- name not found, get default value
                Print.logWarn("JSP name not found: " + jn + " [" + jsp + "]");
                return this._getDefaultWebPageJSP(reqState); // may be null
            }
        }
    }
    ...
    **/
    public String getFaviconLink()
    {
        if (this.faviconLink != null) {
            ...
            return RTConfig.getString(DBConfig.PROP_track_FaviconLink, null);
        }
    }
