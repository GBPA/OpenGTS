...
import org.opengts.war.model.MenuItemModel;
import org.opengts.war.model.MenuGroupModel;
    ...
    implements org.opengts.war.track.Constants
{
    ...
    public static final String MENU_TRACK_DASHBOARD         = "menu.track.dashboard";
        // DashboardMap Map

    public static final String MENU_TRACK_SHARED            = "menu.track.shared";
        // Shared Device Map[X]
    ...
    public static final String MENU_REPORTS_DEVICE_PERFORM  = "menu.rpts.devicePerformance";
        // Device Performance
        //      Excess Speed[X]
        //      Hard Braking
        //      Driving/Stop Time Report[X]
    public static final String MENU_REPORTS_DRIVER_PERFORM  = "menu.rpts.driverPerformance";
        // Driver Performance
        //      Excess Speed[X]
        //      Hard Braking
        //      Driving/Stop Time Report[X]
    ...
    public static void writeTableRow(PrintWriter out, String pageName, RequestProperties reqState)
    {
        ...
        */
        ...
        if (privLabel.getBooleanProperty(PrivateLabel.PROP_MenuBar_usePullDownMenus,DFT_USE_PULL_DOWN_MENUS)) {
            ...
            WebPage mainMenu = pageMap.get(Track.PAGE_MENU_TOP);
            if (mainMenu != null) {
    ...
    /* get the menu bar data model */
    public static void getMenuModel(String pageName, RequestProperties reqState)
    {
        final PrivateLabel privLabel = reqState.getPrivateLabel();
        boolean inclAnchor = privLabel.getBooleanProperty(PrivateLabel.PROP_MenuBar_includeTextAnchor,false);
        ...
        if (privLabel.getBooleanProperty(PrivateLabel.PROP_MenuBar_usePullDownMenus,DFT_USE_PULL_DOWN_MENUS)) {
            ...
            for (String mgn : menuMap.keySet()) {
                ...
                java.util.List<WebPage> pageList = mg.getWebPageList(reqState);
                if (ListTools.isEmpty(pageList)) {
                    // -- this menu does not contain any items
                    continue;
                }

                /* MenuGroupModel */
                String menuName = mgn;
                String menuDesc = mg.getTitle(reqState.getLocale());
                MenuGroupModel mgModel = new MenuGroupModel(menuName, menuDesc);

                /* loop through menu items */
                for (WebPage wp : pageList) {
                    String  title  = wp.getNavigationTab(reqState);
                    String  help   = wp.getMenuHelp(reqState, null);
                    String  url    = wp.encodePageURL(reqState);
                    String  target = StringTools.blankDefault(wp.getTarget(),"_self");
                    boolean window = !target.startsWith("_")? true : false;
                    PixelDimension pixDim = wp.getWindowDimension();
                    int     winW   = (pixDim != null)? pixDim.getWidth()  : -1;
                    int     winH   = (pixDim != null)? pixDim.getHeight() : -1;
                    MenuItemModel miModel = new MenuItemModel(title,help,url,target,window,winW,winH);
                    mgModel.addMenuItem(miModel);
                }
