    ...
    implements WebPage
{
    ...
    public static String getFilteredID(RequestProperties reqState, /*String propKey,*/ String id)
    {
        if (id == null) {
            ...
            if ((privLabel == null) || privLabel.globalValidateIDs()) { //privLabel.getBooleanProperty(propKey,true)
                return StringTools.trim(AccountRecord.getFilteredID(id,false/*nullOnError*/,true/*lowerCase*/,true/*strict*/));
            } else {
    ...
    **/
        ...
        )
    {
        ...
        /* datalist id */
        String dataListID = null;
        if (editable && !ListTools.isEmpty(dataList)) {
            dataListID = !StringTools.isBlank(id)? (id + "_") : "DataList_";
            dataListID += StringTools.createRandomString(8,StringTools.RANDOM_ALPHANUMERIC);
        }
        ...
        if (editable && !StringTools.isBlank(dataListID)) {
            sb.append(" list=\""+dataListID+"\"");
        }
        sb.append(">");

        /* "datalist" (HTML5+, but not Safari) */
        if (editable && !StringTools.isBlank(dataListID)) {
            // -- display: [none | inline-block];
            sb.append("\n");
            sb.append("<datalist id=\"" + dataListID + "\" >\n");
            for (String OPT : dataList) {
                sb.append("  <option label=\""+OPT+"\">"+OPT+"</option>\n");
            }
            sb.append("</datalist>\n");
        }
        ...
        )
    {
        ...
        /* form input */
        sb.append("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_DATA+"'>");
        sb.append(Form_TextField(id, key, editable, value, onclick, size, maxLen, dataList));
    ...
    **/
    ...
    {
        // ie. "Domain.WebPages.TrackMapFleet."
    ...
    public String getRuntimePropertyKey(String key)
    {
        if (!StringTools.isBlank(key) && this.hasRuntimePropertyKeyPrefix()) {
            // ie. ("Domain.WebPages.TrackMapFleet." + KEY)
    ...
    public String getStringProperty(PrivateLabel privLabel, String key, String dft)
    {
        ...
        /* check page specific properties */
        RTProperties rtp = this.getProperties();
        if (rtp.hasProperty(key)) {
            // -- ie. ("showFleetFromCalendar")
            return rtp.getString(key, dft);
        }

        /* check PrivateLabel general properties */
        if (privLabel != null) {
            // -- ie. ("showFleetFromCalendar")
