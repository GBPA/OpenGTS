    ...
    extends BasicPrivateLabelLoader
{
        ...
        OrderedMap<String,String> dftLegend)
    {
        ...
        if (!BasicPrivateLabelLoader._isAttributeActive(active,mapName)) {
            // -- inactive, ignore
        ...
        for (int c = 0; c < attrList.getLength(); c++) {
            ...
            if (attrName.equalsIgnoreCase(TAG_Property)) { // "MapProvider" sub
                ...
                if (!StringTools.isBlank(key)) {
                    ...
                    if (!StringTools.isBlank(keyPrefix)) {
                        String rtpKey = keyPrefix + rtKey;
                        String v = RTConfig.getString(rtpKey, null);
                        if (v != null) {
                            val = v;
                        }
                    }
            ...
            if (attrName.equalsIgnoreCase(TAG_Pushpins)) {
                OrderedMap<String,PushpinIcon> ppMap = this.parseTAG_Pushpins(xmlFile, pl, attrElem, dftPushpinMap, false);
                if (ppMap != null) {
        ...
        /* add global default pushpins */
        /* removed, already handled by attribute "includeDefault" [2.6.4-B35]
        if (!ListTools.isEmpty(dftPushpinMap)) {
            // -- add default pushpins to those defined within the MapProvider
            OrderedMap<String,PushpinIcon> mpPushpins = mp.getPushpinIconMap(null); // TODO: null
            for (String ppKey : dftPushpinMap.keySet()) {
                if (mpPushpins.containsKey(ppKey)) {
                    // -- do not overwrite pushpin definitions already defined by the MapProvider
                } else {
        ...
        */
        ...
        /* add minimum defined Pushpins (black,brown,red,green,...] */
        OrderedMap<String,PushpinIcon> mpPushpins = mp.getPushpinIconMap(null); // TODO: null
        OrderedMap<String,PushpinIcon> minDefPPMap = PushpinIcon.newDefaultPushpinIconMap();
        for (String ppKey : minDefPPMap.keySet()) {
            if (mpPushpins.containsKey(ppKey)) {
                // -- do not overwrite pushpin definitions already defined by the MapProvider
            } else {
                PushpinIcon ppi = /*(PushpinIcon)*/dftPushpinMap.get(ppKey);
    ...
    **/
    protected String parseLegendHTML(String refName, Locale locale, 
        OrderedMap<String,PushpinIcon> pushpins, 
        String legendType, Element legendElem)
    {
        OrderedMap<String,PushpinIcon> pushpinMap = pushpins;
    ...
    *** @return The default pushpin map updated with the pushpins contained within this parsed tag section
    **/
    ...
    {
        OrderedMap<String,PushpinIcon> pushpinMap = new OrderedMap<String,PushpinIcon>();

        /* init with default pushpins? */
        ...
        /* disabled? */
        boolean enabled = XMLTools.getAttributeBoolean(attrElem, ATTR_enabled, true, false);
        if (!enabled) {
            return pushpinMap;
        }

        /* load from image directory? */
        String imageDir = XMLTools.getAttribute(attrElem, ATTR_imageDir, null, false);
        if (!StringTools.isBlank(imageDir)) {
            PushpinIcon.LoadFromDirectory(imageDir,pushpinMap);
            // -- continue below
        }

        /* base URL */
        String baseURL = XMLTools.getAttribute(attrElem, ATTR_baseURL, "", false);
        if (StringTools.isBlank(baseURL) && !StringTools.isBlank(imageDir)) {
            // -- use the same directory as where images were loaded
            baseURL = imageDir;
        }
        // --
        if (!StringTools.isBlank(baseURL) && !baseURL.endsWith("/")) {
                ...
                if (!StringTools.isBlank(key)) {
                    ...
                    if (!alias.equals("")) {
                        ...
                        if (!iconURL.equals("")) {
                            ...
                            if (iconHotspot[0] <= -99) { iconHotspot[0] = iconSize[0] / 2; }
                            if (iconHotspot[1] <= -99) { iconHotspot[1] = iconSize[1]; }
                            // -- image URL
                            ...
                            int backOffset[] = this._parseDim(XMLTools.getAttribute(iconElem,ATTR_backOffset,"-99,-99",false));
                            if (backOffset[0] <= -99) { backOffset[0] = backSize[0] / 2; }
                            if (backOffset[1] <= -99) { backOffset[1] = backSize[1] / 2; }
                            // -- Pushpin
        ...
        } // loop through "Pushpin" tags

        /* return PushpinIcon map */
        return pushpinMap;
    ...
    protected void parseTag_JSPEntries(File xmlFile, String i18nPkgName, BasicPrivateLabel bpl, Element jspFiles)
    {
        ...
        Map<String,PrivateLabel.JSPEntry> jspMap = new OrderedMap<String,PrivateLabel.JSPEntry>();
        PrivateLabel pl  = (PrivateLabel)bpl;
        ...
        for (int j = 0; j < jspList.getLength(); j++) {
            ...
            if (jspElemName.equalsIgnoreCase(TAG_JSP)) {
                ...
                if (StringTools.isBlank(name)) {
                    ...
                    if (StringTools.endsWithIgnoreCase(file,".ftl") && !DBConfig.hasFreemarkerPackage()) {
                        // -- ignore Freemarker .ftl files
                        Print.logDebug("["+xmlFile+"] Ignoring FTL Entry file: " + name);
                    } else {
    ...
    protected void parseTag_WebPages(File xmlFile, String i18nPkgName, BasicPrivateLabel bpl, Element webPages)
    {
        ...
        if (!StringTools.isBlank(keyPrefix)) {
            // -- Examples:
            // -    Domain.WebPages.cssDir=extra/css
            // -    Domain.WebPages.iconDir=extra/images
        ...
        for (int p = 0; p < webPageList.getLength(); p++) {
                    ...
                    if (childName.equalsIgnoreCase(TAG_Property)) { // TAG_Page sub [PageProperties]
                        ...
                        if (!StringTools.isBlank(key)) {
                            ...
                            if (!StringTools.isBlank(rtKeyPfx)) {
                                ...
                                // -  ie. ("Domain.WebPages.TrackMapFleet." + "showFleetFromCalendar")
                            ...
                            rtProps.setProperty(key, val); // page properties
                        } else {
                            printWarn("Undefined property key ignored.");
                            this._setHasParsingWarnings(xmlFile);
                        }
            ...
            if (wpElemName.equalsIgnoreCase(TAG_Link)) {
                ...
                for (int c = 0; c < chList.getLength(); c++) {
                    ...
                    String childName = chNode.getNodeName();
                    Element childElem = (Element)chNode;
                    if (childName.equalsIgnoreCase(TAG_LinkURL)) {
                        String targ = XMLTools.getAttribute(childElem,ATTR_target,null,false);
                        urlLink = StringTools.trim(XMLTools.getNodeText(childElem,"",false));
                        if (!StringTools.isBlank(targ)) {
                            target = targ;
                        }
                    } else
                    if (childName.equalsIgnoreCase(TAG_NavigationDescription)) {
                ...
                wpu.setURL(StringTools.trim(urlLink));
                wpu.setTarget(StringTools.blankDefault(target,"_blank"));
    ...
    protected void parseTag_Reports(File xmlFile, String i18nPkgName, BasicPrivateLabel bpl, Element reports)
    {
        ...
        for (int r = 0; r < reportList.getLength(); r++) {
            ...
            for (int c = 0; c < chList.getLength(); c++) {
                ...
                if (childName.equalsIgnoreCase(TAG_OmitColumn)) {
                    String omitColN = StringTools.trim(XMLTools.getNodeText(childElem,"",false));
                    if (!StringTools.isBlank(omitColN)) {
                        if (omitColNames == null) {
                            omitColNames = new HashSet<String>();
                        }
                        omitColNames.add(omitColN);
                    }
                } else
                if (childName.equalsIgnoreCase(TAG_LogMessage)) {
                    this.parseTag_LogMessage(xmlFile, pl, childElem);
            ...
            try {
                ...
                if (rf != null) {
                    ...
                    ReportEntry re = new ReportEntry(rf, aclName, omitColNames);
    ...
    protected void parseTag_PointsOfInterest(File xmlFile, String i18nPkgName, BasicPrivateLabel bpl, Element pois)
    {
        ...
        for (int p = 0; p < poiNodeList.getLength(); p++) {
            ...
            PoiProvider pp = new PoiProvider() {
                ...
                public int    getPushpinIconIndex(OrderedMap<String,PushpinIcon> iconMap, BasicPrivateLabel bpl) { return EventData._getPushpinIconIndex(icn,iconMap,0); }
