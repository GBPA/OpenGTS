    ...
    extends HttpServlet
{
    ...
    /* version */
    public  static final String  VERSION                        = "2.0.2"; // 2.6.4-B65
    ...
    private static final String  PROP_Events_accCtlAllowOrigin  = "Events.accessControlAllowOrigin";
        ...
        throws ServletException, IOException
    {
        ...
        String acao = RTConfig.getString(PROP_Events_accCtlAllowOrigin,null);
        ...
        throws ServletException, IOException
    {
        ...
        try {
            ...
            if (account == null) {
                ...
                //Audit.userLoginFailed(accountID, userID, ipAddr, privLabel.getName(), null);
                this.errorResponse(response, outFmt, "Invalid authorization");
                return;
            } else
            if (!account.isActive()) {
                Print.logError("Account inactive: " + accountID);
                this.errorResponse(response, outFmt, "Account inactive");
                return;
            } else
            if (account.isExpired()) {
                Print.logError("Account expired: " + accountID);
                this.errorResponse(response, outFmt, "Account expired");
                return;
            } else
            if (account.isSuspended()) {
                Print.logError("Account suspended: " + accountID);
                this.errorResponse(response, outFmt, "Account suspended");
                return;
            }
        ...
        if (!StringTools.isBlank(userID)) {
            ...
            if (user == null)  {
                if (!User.isAdminUser(userID)) {
                    Print.logError("User not found: " + accountID + "/" + userID);
                    //Audit.userLoginFailed(accountID, userID, ipAddr, privLabel.getName(), requestURL);
                    this.errorResponse(response, outFmt, "Invalid authorization");
                    return;
                }
            } else
            if (!user.isActive()) { // [2.6.1-B45]
                Print.logError("User not active: " + accountID + "/" + userID);
                //Audit.userLoginFailed(accountID, userID, ipAddr, privLabel.getName(), requestURL);
                this.errorResponse(response, outFmt, "Invalid authorization");
                return;
            } else
            if (user.isExpired()) { // [2.6.1-B45]
                Print.logError("User expired: " + accountID + "/" + userID);
                //Audit.userLoginFailed(accountID, userID, ipAddr, privLabel.getName(), requestURL);
                this.errorResponse(response, outFmt, "Invalid authorization");
                return;
            }
        ...
        if (user != null) {
            // -- check user password
            if (!user.checkPassword(privLabel,password,true/*suspend*/)) {
                Audit.userLoginFailed(accountID, userID, ipAddr, privLabel.getName(), requestURL);
            ...
            if (!account.checkPassword(privLabel,password,true/*suspend*/)) {
                Audit.userLoginFailed(accountID, userID, ipAddr, privLabel.getName(), requestURL);
