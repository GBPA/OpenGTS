...
// ----------------------------------------------------------------------------
// References:
//  - https://www.mapstraction.com/
//  - https://mapstraction.com/svn/source/
//  - https://www.koders.com/javascript/fid85B6D94DE9D67A1D5648D53ED9E2BC05FFA5CD42.aspx
// ----------------------------------------------------------------------------
// Required supporting JavaScript:
//  - https://mapstraction.com/svn/source/mapstraction-geocode.js
//  - https://mapstraction.com/svn/source/mapstraction.js
//  - https://www.koders.com/javascript/fid85B6D94DE9D67A1D5648D53ED9E2BC05FFA5CD42.aspx
// ----------------------------------------------------------------------------
// Change History:
    ...
    **/
        ...
        throws IOException
    {
        ...
        if (mapProvider.equals(PROVIDER_ESRI)) {
            ...
            WebPageAdaptor.writeCssLink(out, reqState, "https://serverapi.arcgisonline.com/jsapi/arcgis/3.2/js/esri/css/esri.css", null);
            WebPageAdaptor.writeCssLink(out, reqState, "https://serverapi.arcgisonline.com/jsapi/arcgis/3.2/js/dojo/dijit/themes/claro/claro.css", null);
        } else
        if (mapProvider.equals(PROVIDER_LEAFLET)) {
            // - 2013/08/23:
            WebPageAdaptor.writeCssLink(out, reqState, "https://leaflet.cloudmade.com/dist/leaflet.css", null);
        } else
        {
            // ignore
        }
        ...
        throws IOException
    {
        ...
        if (mapProvider.equals(PROVIDER_OPENLAYERS)) {
            // - 2013/09/23
            jsList.add("https://dev.openlayers.org/releases/OpenLayers-2.9.1/OpenLayers.js");
            // - old version below
            //jsList.add("https://openlayers.org/api/OpenLayers.js");
        } else
        if (mapProvider.equals(PROVIDER_GOOGLEV3)) {
            // - 2013/09/23
            jsList.add("https://maps.google.com/maps/api/js?sensor=false");
        ...
        if (mapProvider.equals(PROVIDER_OPENSPACE)) {
            // - 2013/09/23:
            String key = this._getAuthKey(mapProvider,"");
            jsList.add("https://openspace.ordnancesurvey.co.uk/osmapapi/openspace.js?key="+key);
        } else
        // ------
        ...
        if (mapProvider.equals(PROVIDER_OPENMQ)) {
            ...
            // TODO: unable to zoom
            jsList.add("https://open.mapquestapi.com/sdk/js/v7.0.s/mqa.toolkit.js");
        } else
        if (mapProvider.equals(PROVIDER_CLOUDMADE)) {
            // - 2013/09/23: key is required for map display
            jsList.add("https://tile.cloudmade.com/wml/latest/web-maps-lite.js");
        } else
        if (mapProvider.equals(PROVIDER_MICROSOFT)) {
            // TODO: map does not display properly within the "DIV"
            // - 2013/09/23: Bing Maps v6
            jsList.add("https://ecn.dev.virtualearth.net/mapcontrol/mapcontrol.ashx?v=6.3&mkt=en-us");
            // - old version below
            //jsList.add("https://dev.virtualearth.net/mapcontrol/mapcontrol.ashx?v=5");
        } else
        if (mapProvider.equals(PROVIDER_MICROSOFT7)) {
            // TODO: map does not display properly within the "DIV"
            // TODO: key is required for map display
            // - 2013/09/23: Bing Maps v7
            jsList.add("https://ecn.dev.virtualearth.net/mapcontrol/mapcontrol.ashx?v=7.0");
        } else
        if (mapProvider.equals(PROVIDER_MAPQUEST)) {
            // - 2013/08/23: does not appear to be fully supported by Mapstraction
            String key = this._getAuthKey(mapProvider,"");
            jsList.add("https://www.mapquestapi.com/sdk/js/v7.0.s/mqa.toolkit.js?key="+key+"");
            // - old version below
            //String key = this._getAuthKey(mapProvider,"mjtd%7Clu6t210anh%2Crn%3Do5-labwu");
            //jsList.add("https://btilelog.access.mapquest.com/tilelog/transaction?transaction=script&key="+key+"&ipr=true&itk=true&v=5.1");
        } else
        if (mapProvider.equals(PROVIDER_ESRI)) {
            // - 2013/08/23: ArcGIS (does not appear to be fully supported by Mapstraction)
            jsList.add("https://serverapi.arcgisonline.com/jsapi/arcgis/?v=3.2");
        } else
        if (mapProvider.equals(PROVIDER_LEAFLET)) {
            // - 2013/08/23: zoom not displayed
            jsList.add("https://leaflet.cloudmade.com/dist/leaflet.js");
        } else
        if (mapProvider.equals(PROVIDER_OVI)) {
            // - 2013/08/23: "ovi.mapsapi.map is undefined"
            jsList.add("https://api.maps.ovi.com/jsl.js");
        } else
        // ------
        // not fully supported
        //
        ...
        if (mapProvider.equals(PROVIDER_FREEEARTH)) {
            ...
            // - old version below
            //jsList.add("https://freeearth.poly9.com/api.js");
        } else
        if (mapProvider.equals(PROVIDER_MAP24)) {
            ...
            //jsList.add("https://api.maptp.map24.com/ajax?appkey="+key);
        } else
        if (mapProvider.equals(PROVIDER_MULTIMAP)) {
            ...
            // - old version below
            //String key = this._getAuthKey(mapProvider,"");
            //jsList.add("https://developer.multimap.com/API/maps/1.2/"+key);
        } else
        if (mapProvider.equals(PROVIDER_YAHOO)) {
            ...
            //jsList.add("https://api.maps.yahoo.com/ajaxymap?v=3.0&appid="+key);
        } else
        // ------
        // unrecognized
        {
        ...
        if (this.getProperties().getBoolean(PROP_LOCAL_JS,false)) {
            ...
            // - old version below
            //jsList.add("https://mapstraction.com/svn/source/mapstraction-geocode.js");
            //jsList.add("https://mapstraction.com/svn/source/mapstraction.js");
        }
