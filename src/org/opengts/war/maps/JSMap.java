    ...
    extends MapProviderAdapter
{
    ...
    **/
    private static final Object SKIP_PROPS[] = new Object[] {
        PROP_javascript_include,
        PROP_javascript_inline,
        PROP_createMapCallback,
        PROP_postMapInitCallback,
        PROP_zone_map_width,
    ...
    **/
    ...
    {
        double lat = RTConfig.getDouble(BasicPrivateLabel.LAF_Map_Default_Latitude , 0.0);
        double lon = RTConfig.getDouble(BasicPrivateLabel.LAF_Map_Default_Longitude, 0.0);
        if (GeoPoint.isValid(lat,lon)) {
            return new GeoPoint(lat, lon);
        } else {
            return super.getDefaultCenter(dft);
        }
    }
    ...
    **/
    ...
    {
        int zoom = RTConfig.getInt(BasicPrivateLabel.LAF_Map_Default_Zoom , -1);
        if (zoom >= 0) {
            return zoom;
        } else {
            return super.getDefaultZoom(dft);
        }
    }
        ...
        throws IOException
    {
        ...
        double       dftZoom      = this.getDefaultZoom(JSMap.DEFAULT_ZOOM);
        double       pushpinZoom  = this.getPushpinZoom(JSMap.PUSHPIN_ZOOM);
        ...
        out.write("// Map Init Callbacks\n");
        JavaScriptTools.writeJSVar(out, "CREATE_MAP_CALLBACK"   , rtp.getString(PROP_createMapCallback,"null"),false);
        JavaScriptTools.writeJSVar(out, "POST_MAP_INIT_CALLBACK", rtp.getString(PROP_postMapInitCallback,"null"),false);
        ...
        JavaScriptTools.writeJSVar(out, "DEFAULT_ZOOM"          , dftZoom);
        JavaScriptTools.writeJSVar(out, "PUSHPIN_ZOOM"          , pushpinZoom);
        ...
        for (Iterator<String> k = iconMap.keyIterator(); k.hasNext();) {
            ...
            if (!StringTools.isBlank(B)) {
                ...
                if (B.startsWith("#") || B.startsWith("%")) {
                    String val = B.substring(1);
                    int bgx = PushpinIcon.getPushpinIconIndex(val,iconMap,-1); // index of PushpinIcon
                    if (bgx < 0) { bgx = 0; } // black
                    out.write(" bgPP:"+bgx+",");
                } else {
                    out.write(" bgURL:\""+B+"\",");
                }
        ...
        throws IOException
    {
        ...
        String jsMapURLs[] = StringTools.parseStringArray(this.getProperties().getString(PROP_javascript_include,""),'\n');
    ...
    public void writeMapUpdate(
        ...
        throws IOException
    {
        super.writeMapUpdate( // XML/JSON format specified by caller
            mapDataFormat,
            reqState);
    }
