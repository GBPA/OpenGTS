...
public class DBAdmin
{
    ...
    **/
    protected static <T extends DBRecord<T>> DBFactory<T> _createDBRecordFactory(String className, boolean isRequired)
    {
        ...
        try {
            ...
            DBFactory<T> dbFact = (DBFactory<T>)(new MethodAction(className,"getFactory")).invoke();
            if (dbFact != null) {
    ...
    **/
    public static <T extends DBRecord<T>> boolean addTableFactory(String className, boolean isRequired)
    {
        DBFactory<T> fact = DBAdmin._createDBRecordFactory(className, isRequired);
        if (fact != null) {
    ...
    **/
    public static <T extends DBRecord<T>> void addTableFactory(DBFactory<T> factory)
    {
        if (factory == null) {
    ...
    **/
    public static boolean includeFieldInFactory(DBFactory<? extends DBRecord<?>> fact, DBField field)
    {
        // TODO:
        return true;
    }
    ...
    **/
    ...
    /**
    *** Gets all child/dependent table factories for the specified table factory<br>
    *** (Dependent factories are sorted in parent hiararchy order)
    *** @param parentFactory  The parent table factory
    *** @return The child/dependent table factories of the specified parent factory
    **/
    @SuppressWarnings("unchecked")
    public static <T extends DBRecord<T>> DBFactory<T>[] getChildTableFactories(DBFactory<T> parentFactory)
    {
        // -- get list of all dependent children/grandchildren
        ...
        //@SuppressWarnings("unchecked")
        DBFactory<T>[] dbf = (DBFactory<T>[])list.toArray(new DBFactory<?>[list.size()]);
        return dbf; // "dbf" var required here, otherwise compiler complains
    }

    /**
    *** Gets all child/dependent table factories for the specified table factory
    *** (Dependent factories are turned in top-down order)
    *** @param parentFactory  The parent table factory
    *** @return The child/dependent table factories of the specified parent factory
    **/
    /* DO NOT USE
    private static <T extends DBRecord<T>> DBFactory<T>[] getChildTableFactories(DBFactory<T> parentFactory)
    {
        OrderedSet<DBFactory<? extends DBRecord<?>>> list = new OrderedSet<DBFactory<? extends DBRecord<?>>>(false);
        DBAdmin._getChildTableFactories(parentFactory, list);
        @SuppressWarnings("unchecked")
        DBFactory<T>[] dbf = (DBFactory<T>[])list.toArray(new DBFactory<?>[list.size()]);
        return dbf; // "dbf" var required here, otherwise compiler complains

    }
    private static <T extends DBRecord<T>> void _getChildTableFactories(DBFactory<T> fact, OrderedSet<DBFactory<? extends DBRecord<?>>> list)
    {
        if (fact != null) {
            DBFactory<? extends DBRecord<?>> child[] = fact.getChildFactories();
            for (int i = 0; i < child.length; i++) {
                // -- may already have been added (re-adding pushes it down in the list)
                list.add(child[i]);
                // -- recursive descent into child
                DBAdmin._getChildTableFactories(child[i], list);
            }
        }
    }
    */
    ...
    /**
    *** Gets all table factories having a record class that subclasses from the specified class
    *** @param tableClass  The table subclass
    *** @return The table factories having a record class that subclasses from the specified class
    **/
    ...
    **/
    ...
    public  static final String ARG_DUMP_FORMAT[]   = new String[] { "dumpFormat", "dumpFmt" };
  ...
  //public  static final String ARG_RELOAD[]        = new String[] { "reload"        };
    ...
    **/
    //@SuppressWarnings({"rawtypes","unchecked"})  // <? extends DBRecord<?>>
    //@SuppressWarnings("all")
    public static <T extends DBRecord<T>> DBAdminExec execCommands() // may not build on Java6
    {
        ...
        if (reloadTable != null) {
            ...
            // db factory
            DBFactory<T> fact = DBAdmin.getTableFactory(reloadTable);
            if (fact == null) {
            ...
            try {
                fact.dumpTable(reloadFile, append, null);
            } catch (DBException dbe) {
        ...
        */
        ...
        if (dumpTable != null) {
            ...
            if (dumpTable.equalsIgnoreCase(DUMP_ALL) || StringTools.isBlank(dumpTable)) {
                // -dump=all
            ...
            // -- dump format
            String dumpFmt = RTConfig.getString(ARG_DUMP_FORMAT,null);
            if (!StringTools.isBlank(dumpFmt)) {
                if (dumpFmt.equalsIgnoreCase("csv")) {
                    dftExt = DBFactory._DUMP_EXT_CSV;
                } else
                if (dumpFmt.equalsIgnoreCase("txt")) {
                    dftExt = DBFactory._DUMP_EXT_TXT;
                } else
                if (dumpFmt.equalsIgnoreCase("xml")) {
                    dftExt = DBFactory._DUMP_EXT_XML;
                } else {
            ...
            for (Iterator<String> i = tableList.iterator(); i.hasNext(); ) {
                ...
                // -- DBFactory for table
                @SuppressWarnings("unchecked")
                DBFactory<T> fact = (DBFactory<T>)DBAdmin.getTableFactory(utableName);
                if (fact != null) {
                    try {
                        DBSelect<T> dbSel = new DBSelect<T>(fact,where); // "unchecked call"
                        fact.dumpTable(dumpFile, append, dbSel); // "unchecked conversion"
                    } catch (DBException dbe) {
