...
public class DBSelect<gDBR extends DBRecord<gDBR>>
{
        ...
        if (ListTools.isEmpty(sf)) {
            ...
            if (fact != null) {
                for (int i = 0; i < sf.length; i++) {
                    if (sf[i] == null) {
                        Print.logError("DBFactory field is null: %s.%s", this.getUntranslatedTableName(), sf[i]);
                    } else
                    ...
                    if (sf[i].equalsIgnoreCase(DBProvider.FLD_COUNT())) {
                        ...
                        Print.logError("DBFactory field does not exist: %s.%s", this.getUntranslatedTableName(), sf[i]);
                    }
                }
            }
    ...
    **/
    ...
    {
        return ((this.selectFields != null) && (this.selectFields.length > 0))? true : false;
    }

    /**
    *** Gets the number of exlicitly selected fields
    *** @return The number of exlicitly selected fields
    **/
    public int getSelectedFieldsCount()
    {
        return this.hasSelectedFields()? this.selectFields.length : 0;
    }
    ...
    **/
    ...
    {
        return this.hasSelectedFields()? this.selectFields : null;
    }
    ...
    *** @param gbf An array of group-by fields
    **/
    public void setGroupByFields(String... gbf)
    {
        if ((gbf == null) || (gbf.length == 0)) {
            ...
            DBFactory<gDBR> fact = this.getFactory();
            if (fact != null) {
                for (int i = 0; i < gbf.length; i++) {
                    if (gbf[i] == null) {
                        Print.logError("DBFactory field is null: %s.%s", this.getUntranslatedTableName(), gbf[i]);
                    } else
                    if (fact.hasField(gbf[i])) {
                        // -- ok as-is
                    } else {
                        Print.logError("DBFactory field does not exist: %s.%s", this.getUntranslatedTableName(), gbf[i]);
                    }
                }
            }
            this.groupByFields = gbf;
        }
    }

    /**
    *** Returns true if this DBSelect has group-by fields defined
    *** @return True if this DBSelect has group-by fields defined
    **/
    public boolean hasGroupByFields()
    {
        return (this.groupByFields != null);
    }

    /**
    *** Gets the group-by fields
    *** @return An array of group-by fields, or null if no group-by fields have been defined
    **/
    public String[] getGroupByFields()
    {
        return ((this.groupByFields != null) && (this.groupByFields.length > 0))? this.groupByFields : null;
    }
        ...
        if ((obf == null) || (obf.length == 0)) {
            ...
            if (fact != null) {
                for (int i = 0; i < obf.length; i++) {
                    if (obf[i] == null) {
                        Print.logError("DBFactory field is null: %s.%s", this.getUntranslatedTableName(), obf[i]);
                    } else
                    if (fact.hasField(obf[i])) {
                        // ok as-is
                    } else {
                        Print.logError("DBFactory field does not exist: %s.%s", this.getUntranslatedTableName(), obf[i]);
                    }
                }
            }
    ...
    **/
    private String _toString(DBProvider dbp, boolean inclSelect)
    {
        ...
        /* GROUP BY */
        if (this.hasGroupByFields()) {
            sb.append(" GROUP BY ");
            String fld[] = this.getGroupByFields();
            for (int i = 0; i < fld.length; i++) {
                if (i > 0) {
                    sb.append(",");
                }
