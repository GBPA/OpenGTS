...
//     -Added support for alternate index key sort-order (see "altNdxSort")
// ----------------------------------------------------------------------------
...
public class DBField
{
    ...
    **/
    ...
    public static String TYPE_COLOR()               { return DBField.TYPE_STRING("color"        , 20); }
    public static String TYPE_TIREPRESS()           { return DBField.TYPE_STRING("tirePressure" ,750); }
    public static String TYPE_TIRETEMP()            { return DBField.TYPE_STRING("tireTemp"     ,750); }
    ...
    **/
    ...
    private String                              altNdxNames[]   = null; // alternate index names
    private int                                 altNdxSort[]    = null; // alternate index sort-order
    ...
    **/
        ...
        String charSet, Set<String> indexNames)
    {
        ...
        /* alternate index */
        this.altNdxNames = null;
        this.altNdxSort  = null;
        if ((indexNames != null) && !indexNames.isEmpty()) {
            String insa[]    = indexNames.toArray(new String[indexNames.size()]);
            this.altNdxNames = new String[insa.length];
            this.altNdxSort  = new int   [insa.length];
            boolean hasSort  = false;
            for (int i = 0; i < insa.length; i++) {
                String ns = insa[i]; // ie. "altNdx:2"
                int p = ns.indexOf(":");
                if (p >= 0) {
                    // -- found sort-order specification
                    this.altNdxNames[i] = ns.substring(0,p);
                    this.altNdxSort [i] = StringTools.parseInt(ns.substring(p+1),-2);
                    if (this.altNdxSort[i] >= 0) {
                        hasSort = true;
                    }
                } else {
                    // -- no sort-order specification
                    this.altNdxNames[i] = ns;
                    this.altNdxSort [i] = -1;
                }
            }
            // -- optimization, clear if no sort-order is defined
            if (!hasSort) {
                this.altNdxSort = null;
            }
        }
        ...
        DBFactory<? extends DBRecord<?>> fact = DBFactory.getFactoryByName(utableName);
        if (fact != null) {
    ...
    **/
    public DBField(String fldName, Class<?> javaClass, String dataType, I18N.Text title, String attr)
    {
        ...
        /* data type */
        String fieldDT   = (dataType != null)? dataType.toUpperCase() : "";
        this.dataType    = fieldDT;
        this.typeMask    = DBProvider.getDataTypeMask(this.dataType);

        /* character set */
        this.charSet     = (RTConfig.getBoolean(RTKey.DB_UTF8) && this.getBooleanAttribute(ATTR_UTF8,false))? UTF8_CHARACTER_SET : "";
        ...
        if ((akn != null) && (akn.length > 0)) {
            ...
            for (int i = 0; i < akn.length; i++) {
                ...
                if (ak.equals(DBProvider.DEFAULT_ALT_INDEX_NAME)) {
                    // -- explicit 'altIndex' default value
                    //Print.logInfo("Alternate Index: " + this._getName() + " ==> " + ak);
                    altKeyList.add(ak);
                } else
                if (Character.isLetter(ak.charAt(0))) { // must begin with alpha/letter
                    // -- other alternate index name
                    //Print.logInfo("Alternate Index: " + this._getName() + " ==> " + ak);
                    altKeyList.add(ak);
                } else {
                    Print.logError("Invalid alternate key name: " + ak);
                }
            }
            // --
            if (!ListTools.isEmpty(altKeyList)) {
                String insa[]    = altKeyList.toArray(new String[altKeyList.size()]);
                this.altNdxNames = new String[insa.length];
                this.altNdxSort  = new int   [insa.length];
                boolean hasSort  = false;
                for (int i = 0; i < insa.length; i++) {
                ...
                Print.logError("Invalid alternate key specification for field: " + fldName);
            }
        }

        /* required/optional? */
        if (this.isPriKey || this.isUniqueAltKey || (this.altNdxNames != null)) {
    ...
    **/
    public void setFactory(DBFactory<? extends DBRecord<?>> factory)
    {
    ...
    **/
    ...
    {
        return !ListTools.isEmpty(this.altNdxNames)? true : false;
    }
    ...
    *** @return An array of alternate key index names, or null if this field has no alternate keys
    **/
    public String[] getAlternateIndexes()
    {
        return this.isAlternateKey()? this.altNdxNames : null;
    }

    /**
    *** Gets the alternate index sequence sort-order
    *** @param altKey  The alternate index name
    *** @return The alternate index sequence (< 0 if no sequence is defined)
    **/
    public int getAlternateIndexSequence(String altKey)
    {
        if (!ListTools.isEmpty(this.altNdxSort)) {
            int p = ListTools.indexOfIgnoreCase(this.altNdxNames, altKey);
            return (p >= 0)? this.altNdxSort[p] : -3;
        } else {
            return -1;
        }
    }
    ...
    **/
    ...
    {
        DBFactory<? extends DBRecord<?>> dbf = this.getFactory();
    ...
    **/
    public Class<? extends Enum<?>> getEnumClass()
    {
        if (!this.enumInit) {
            ...
            if (!StringTools.isBlank(enumValue)) {
                if (enumValue.indexOf("$") < 0) {
                    DBFactory<? extends DBRecord<?>> fact = this.getFactory();
                    if (fact != null) {
        ...
        return (Class<? extends Enum<?>>)this.enumClass; // may be null
    }
