...
//     -Added "toJSON" to return a JSON object of the target record.
// ----------------------------------------------------------------------------
...
public abstract class DBRecord<gDBR extends DBRecord<gDBR>>
{
    ...
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBFactory<T> fact)
        throws DBException
    {
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBFactory<T> fact, StringBuffer where)
        throws DBException
    {
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBFactory<T> fact, String where)
        throws DBException
    {
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBFactory<T> fact, DBWhere where)
        throws DBException
    {
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBSelect<T> dsel)
        throws DBException
    {
    ...
    **/
    ...
    /* package */ static <T extends DBRecord<T>> T _createDBRecord(DBRecordKey<T> rcdKey)
        throws DBException
    {
  ...
  //private   DBFieldValues             fieldVals               = null;
    ...
    **/
    ...
    public DBRecordKey<gDBR> getRecordKey()
    {
        if (this.recordKey == null) {
            ...
            try {
                this.recordKey = /*(DBRecordKey<gDBR>)*/this.getFactory(true).createKey(); // may throw DBException
                this.recordKey._setDBRecord(this); // "unchecked cast"
    ...
    **/
    public static <T extends DBRecord<T>> DBFactory<T> getFactory(DBRecord<T> dbr)
    {
    ...
    **/
    ...
    {
        DBFactory<? extends DBRecord<?>> fact = this._getFactory();
        if (fact != null) {
    ...
    **/
    @SuppressWarnings("unchecked")
    public static <T extends DBRecord<T>> T[] getNextGroup(DBFactory<T> fact, ResultSet rs, int max)
        throws DBException
    {
    ...
    **/
    @SuppressWarnings("unchecked")
    public static <T extends DBRecord<T>> DBRecord<T>[] _getRecords(DBSelect<T> dsel, 
    ...
    *** @throws DBException If a DB access error occurs
    **/
    public static <T extends DBRecord<T>> T[] getRecords(DBFactory<T> fact, 
        String where,
    ...
    *** @throws DBException If a DB access error occurs
    **/
    protected static <T extends DBRecord<T>> T[] select(DBFactory<T> fact, 
        String where, String addtnlSel, 
        String orderBy[], boolean ascending,
        long limit, long offset,
        DBRecordHandler<T> rcdHandler)
    ...
    *** @throws DBException If a DB access error occurs
    **/
    protected static <T extends DBRecord<T>> T[] select(DBSelect<T> dsel)
        throws DBException
    {
    ...
    **/
    ...
    {
        public <T extends DBRecord<T>> void fieldChanged(DBRecord<T> rcd, String fieldName);
    }
        ...
        throws DBException
    {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                // -- not a fatal error
                Print.logDebug("Key not found: [" + recKey.getUntranslatedTableName() + "] " + wh);
                return null;
            }
        } catch (SQLException sqe) {
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getLastUpdateTime(DBFactory<T> factory)
        throws DBException
    {
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            ...
            /* last insert time */
            String insAcctID = DBRecord.GetCurrentAccount();
            String insUserID = DBRecord.GetCurrentUser();
            //if (!StringTools.isBlank(insAcctID)) {
                this.setLastUpdateTime(nowTime);
                this.setLastUpdateAccount(insAcctID,true);
                this.setLastUpdateUser(insUserID,true);
            //}
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            ...
            //if (!StringTools.isBlank(updAcctID)) {
    ...
    **/
    ...
    //    DBConnection dbc = null;
    //    try {
    //        dbc  = DBConnection.getDBConnection_readWrite(); // depends on prior operation
    ...
    **/
    ...
    //    DBConnection dbc = null;
    //    try {
    //        dbc  = DBConnection.getDBConnection_write(); // depends on prior operation
    ...
    //    DBConnection dbc = null;
    //    try {
    //        dbc  = DBConnection.getDBConnection_write(); // depends on prio operation
    ...
    **/
        ...
        boolean soapXML)
    {
        ...
        /* begin Record tag */
        // -- <Record table="TABLE" partial="true" sequence="SEQUENCE">
        sb.append(PFX1);
    ...
    /**
    *** Encodes this DBRecord into JSON
    *** @param fldNames  The set of field names to include
    *** @param inclBlank Include fields with blank values
    *** @param inclType  Include data-type
    *** @param inclTitle Include field title
    *** @return The StringBuffer containing the XML
    **/
    public JSON._Object toJSON(Set<String> fldNames, int inclMask)
    {
        DBRecordKey<gDBR> recKey     = this.getRecordKey();
        String            utableName = recKey.getUntranslatedTableName();
        DBField           fld[]      = recKey.getFields();       // ALL fields
        ...
        /* what to include */
        boolean inclTable = ((inclMask & DBFactory.INCL_TABLE) != 0);
        boolean inclBlank = ((inclMask & DBFactory.INCL_BLANK) != 0);
        ...
        JSON._Object flds;
        if (inclTable) {
            obj.addKeyValue("table", utableName);
            if (!ListTools.isEmpty(fldNames)) {
                obj.addKeyValue("partial", true);
            }
            flds = new JSON._Object();
            obj.addKeyValue("fields", flds);
        } else {
            flds = obj;
        }
        ...
        for (int i = 0; i < fld.length; i++) {
            if (fld[i].isPrimaryKey()) {
                Object value = fldVals.getFieldValue(fld[i].getName()); // getFieldValueAsString
                flds.addKeyValue(DBFactory.writeJSON_DBField(fld[i], inclMask, value));
            }
        }
        ...
        for (int i = 0; i < fld.length; i++) {
            if (!fld[i].isPrimaryKey()) {
                ...
                Object value = fldVals.getFieldValue(name); // getFieldValueAsString
                if (!inclBlank) {
                    if (value == null) {
                        continue;
                    } else
                    if ((value instanceof String) && StringTools.isBlank((String)value)) {
                        continue;
                    } else
