    ...
    implements ReverseGeocodeProvider, SubdivisionProvider, GeocodeProvider
{
    ...
    public static String getPostalCodeLocation_xml(String postalCode, String country, int timeoutMS)
    {
        ...
        /* get XML String */
        try {
            byte xml[] = HTMLTools.readPage_GET(
                new URL(url),
                timeoutMS, -1);
    ...
    **/
    public ReverseGeocode getReverseGeocode(GeoPoint gp, String localeStr, boolean cache)
    {
        ...
        if (StringTools.isBlank(primaryService)) {
            // -- findNearestAddress (blank default)
            rg = this.getAddressReverseGeocode(gp, localeStr);
        } else
        if (primaryService.equalsIgnoreCase("findNearestAddress")) {
            // -- findNearestAddress
            rg = this.getAddressReverseGeocode(gp, localeStr);
        } else
        ...
        if (primaryService.equalsIgnoreCase("findNearbyPlaceName")) {
            ...
            // -- findNearestAddress (unknown default)
            rg = this.getAddressReverseGeocode(gp, localeStr);
        }
        if (rg != null) {
            return rg;
    ...
    **/
    private ReverseGeocode createAddress(Map<String,String> addrProps, double maxAddressDistanceKM)
    {
        ...
        if (isUS && (state.length() > 2)) {
            String stateCode = USState.getCode(state,null);
    ...
    **/
    private String appendAddressKey(StringBuffer sb, Map<String,String> addrProps, String key, boolean suffixComma)
    {
        String elem = addrProps.get(key);
    ...
    **/
    ...
    private String appendPostalCodeElement(StringBuffer sb, Map<String,String> codeProps, String key, int maxLen, boolean suffixComma)
    {
        String elem = codeProps.get(key);
        if (!StringTools.isBlank(elem)) {
            // -- trim postal code
    ...
    **/
    protected static Document GetXMLDocument(String url, int timeoutMS)
    {
        try {
            ...
            InputStream input = HTMLTools.inputStream_GET(url, null, timeoutMS);
